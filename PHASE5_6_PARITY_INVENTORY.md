...
| --- | --- | --- | --- | --- |
| Focused request attribution (Phase 5 foundation) | `focusedDrill.js` `validateFocusedDrillAttribution` | `directCorrectionCheckpoint.js` `validateExactAttribution` | `C T R I P`; exact assertion schema, actor, source and communication basis | creator/input/result, actor, relation, ingestion, order, replay |
...
| User-facing explanation | `laterOutcome.js` `validateExplanationClosure` | `laterOutcomeCheckpoint.js` `findExactExplanation` | `C T G P`; exact SUT template, bounded evaluator commitments and prohibitions | extra identity, causal/global/permanent/style/efficacy/fatigue/focused/hidden claims |
