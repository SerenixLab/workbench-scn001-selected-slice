...
| `ENG-CONF-AUTHORITY-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7B implements immutable qualification plans/results, mechanically derived campaign authorization, outcome-independent attempt/replacement allocation with separate seed commitment, anchor/fresh-start prerequisites, invalidity, and governed decisions. Corrective Phase 7C/7D replay pins an Ed25519 verifier and grants a start only after authenticated canonical receipt bytes, typed fresh-start proof, configuration, qualification, and authenticated effective namespace-history resolution. The internal attempt pipeline additionally requires the signed start to bind its actual recorder-owned run scope before execution. | `ADR-004 R3`; `ADR-005 R2`; `ADR-011 R3`; `ADR-012 R3` | Contract, negative, replay, and regression tests over authority derivation, replacement seed closure, full typed authorization reconstruction, exact indexed bindings, signed receipt/start custody, namespace history, substituted-key, signed-payload/run-scope attacks, and preflight forgery. | `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/formalAuthority.js`; `scn001_eval/src/formalEvidence.js`; `scn001_eval/src/formalRunRecord.js`; `scn001_eval/src/selectedSliceFormalPipeline.js`; corresponding tests; Phase 7B-7D and corrective records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Synthetic tests exercise start and replay transitions, but sealable invalidity-to-replacement execution and accepted contingency/supersession continuation are not end-to-end orchestrated; no real effective namespace, external anchor event, or independent exact-commit review exists. |
...
| `ENG-CONF-EVIDENCE-002` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7C/7D implement immutable closed evidence envelopes; exact raw-byte custody/replay; recorder-owned attempt chronology; pre-proof raw initial inspection; canonical typed control proofs; cumulative namespace/frozen campaign indexes; complete attempt inventories; durable start authority; and sealed formal run records with exact evidence, control, simulator, allocation, outcome, and failure-finding closure. | `ADR-002 R2`; `ADR-008 R2`; `ADR-011 R3` | Contract, negative, replay, and regression tests cover bytes, envelopes, label-only proof rejection, caller-order rejection, privacy, simulator fidelity, authority reconstruction, namespace history/forks, attempt closure, run evidence partition/order, raw initial-snapshot reconstruction, run isolation, prospective selection, orthogonal validity/invariant/obligation outcomes, and exact findings. | `scn001_eval/src/formalEvidence.js`; `scn001_eval/src/formalRunRecord.js`; corresponding tests/support; Phase 7C/7D records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Tests use synthetic objects only; pre-seal interruption remains abandoned rather than upgraded into a formal run, no test artifact is promotable, and exact-commit independent review is pending. |
...
A pre-seal interruption is retained and produces a valid
`authority_review_required` suspension without inventing a run, invalidity
decision, or replacement.

The external suggestion to seal every interruption was not adopted as a
blanket rule: evidence that cannot satisfy formal-run integrity closure cannot
be upgraded into a sealed invalid record merely to unlock replacement
authority. Sealable `INVALID_UNSCORABLE` records and deterministic replacement
allocation exist as mechanisms, but their end-to-end orchestration through
replacement execution, stop limits, and accepted contingency/supersession
...
is `PHASE7_HAPPY_PATH_PIPELINE_IMPLEMENTED`. The four Phase 7 controls remain
