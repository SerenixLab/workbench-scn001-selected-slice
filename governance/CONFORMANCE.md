...
| `ENG-CONF-AUTHORITY-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7B implements immutable qualification plans/results, mechanically derived campaign authorization, outcome-independent attempt/replacement allocation, structural anchor/fresh-start prerequisites, invalidity, and governed decisions. Phase 7C can convert that closed preflight into a start grant only after exact durable receipt, raw fresh-start, configuration, qualification, and single-head namespace resolution. | `ADR-004 R3`; `ADR-005 R2`; `ADR-011 R3`; `ADR-012 R3` | Contract, negative, replay, and regression tests over authority derivation plus full typed authorization reconstruction, exact indexed bindings, raw receipt/start custody, namespace-head closure, and preflight-forgery attacks. | `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/formalAuthority.js`; `scn001_eval/src/formalEvidence.js`; `scn001_eval/test/formalAuthority.test.js`; `scn001_eval/test/formalEvidence.test.js`; Phase 7B/7C records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Synthetic tests exercise the start transition, but no real effective namespace, external anchor event, campaign execution, or independent exact-commit review exists. |
...
| `ENG-CONF-EVIDENCE-002` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7C implements immutable closed evidence envelopes; exact raw-byte content-addressed custody and replay; independently validated cumulative namespace/frozen campaign indexes; complete authorized-attempt inventories; and durable single-head execution-authority resolution. | `ADR-002 R2`; `ADR-008 R2`; `ADR-011 R3` | Contract, negative, replay, and regression tests cover byte normalization/reconstruction, canonical storage, envelope closure, evaluator leakage, simulator fidelity, missing custody, preflight forgery, configuration/qualification/anchor binding, stale heads, forks, history deletion, attempt omission, and member mismatch. | `scn001_eval/src/formalEvidence.js`; `scn001_eval/test/formalEvidence.test.js`; `scn001_eval/test_support/formalFixtures.js`; Phase 7C record below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Formal run records and bounded scoring remain unimplemented until 7D; tests use synthetic objects only, no actual external anchor/campaign exists, and exact-commit independent review is pending. |
...
| `ENG-CONF-SCORE-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | not-applicable | No formal path/class aggregator, run-count selector, bounded campaign result/standing, or bounded claim artifact exists. Future trigger: first implementation of any such artifact or operation. | `ADR-004 R3`; `ADR-005 R2`; `ADR-009 R4`; `ADR-011 R3`; `ADR-012 R3` | Repository and claim inventory; bounded-result and claim-ceiling review. | `README.md`; `PHASE7_READINESS.md`; accepted ADR-012 snapshot | Not applicable until trigger; then local-recorded plus CI-required controls are mandatory. | N/A | merge-blocking; promotion-blocking; claim-blocking | Applicability must change before classification, aggregation, result/standing, or bounded-claim implementation; current tests and checkpoints are not scoreable campaign results. |
...
| `ENG-HEALTH-ABSTRACTION-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Existing bounded SUT/checkpoint abstractions remain unchanged; Phase 7 separates canonical/configuration identity, prospective authority, and durable evidence/index custody into evaluator-owned modules. Scoring remains absent and separate. | `ADR-001 R1` | Responsibility review plus dependency and hostile suites verify that persistence resolves exact authority artifacts without importing private SUT code or changing SUT state. | Existing bounded modules; `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/configurationIdentity.js`; `scn001_eval/src/formalAuthority.js`; `scn001_eval/src/formalEvidence.js`; tests | Implementing-task responsibility review; fresh independent Phase 7A-7C structure review pending. | uncovered | advisory or merge-blocking by condition | The bounded file-backed store and indexes are locally justified but not independently reviewed; 7D must preserve evidence/scoring separation. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Phase 7A/7B are committed focused increments; Phase 7C adds only durable evidence, namespace/campaign indexes, and the deferred start-resolution boundary, with no score or real campaign execution. | Rule entry | Section diff; repeated hostile passes; targeted authority/evidence suites; full local gates; historical reviews remain scoped to their named ranges. | Phase 7A-7C source/tests and records; prior review closures | Implementing-task red-team and local gates; fresh independent exact-commit reviews pending. | uncovered | advisory or merge-blocking by condition | The implementing task cannot independently attest its Phase 7 commits; protected required-check configuration remains unverified. |
...
| `ENG-HEALTH-FAILURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Malformed or ambiguous bounded state still fails before mutation; Phase 7C additionally fails closed on malformed evidence, custody, indexes, authorization reconstruction, missing history, and namespace conflict without granting execution authority. | Rule entry | Existing typed/rollback attacks plus exact-byte, canonical-file, closed-envelope, actor, reference, completeness, cumulative-history, fork, stale-head, and forged-preflight attacks. | SUT/evaluation source and package tests; `scn001_eval/src/formalEvidence.js`; `scn001_eval/test/formalEvidence.test.js`; repository gates | Default plus formal-evidence local-recorded/CI-required; protected-check verification pending. | uncovered | merge-blocking; claim-blocking | Local file writes can leave harmless unreferenced content-addressed bytes after an environmental failure between byte and envelope writes; no incomplete artifact receives a reference or authority. |
...
| `ENG-HEALTH-STRUCTURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | SUT closure/checkpoints remain unchanged; configuration, prospective authority, and durable evidence/index ownership are separate evaluator modules, and only the durable resolver may grant a previously reconstructed preflight. | Rule entry | Static dependency direction; closed construction by artifact family; actual-artifact resolution at authority transitions; targeted boundary/authority/evidence attacks. | Existing bounded modules; Phase 7A-7C modules/tests; dependency gate | Implementing-task structural review; fresh independent Phase 7 structure review pending. | uncovered | advisory or merge-blocking by condition | The new persistence/index boundary requires independent review; 7D must not merge scoring authority into capture or SUT ownership. |
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Regressions cover the complete development trajectory and Phase 7A-7C configuration, authority, raw custody/replay, typed evidence, durable start, namespace, and campaign-index mechanisms. | Rule entry | Existing behavior suites plus Phase 7 contract, negative, property, replay, fork, completeness, privacy, and regression tests; full aggregate gate. | SUT/evaluation tests; `scn001_eval/test/configurationIdentity.test.js`; `scn001_eval/test/formalAuthority.test.js`; `scn001_eval/test/formalEvidence.test.js`; conformance tests; `package.json` | Default plus formal-evidence local-recorded/CI-required; protected-check verification pending. | uncovered | merge-blocking; promotion-blocking | Phase 7A-7C mechanisms are locally tested but await fresh independent review; formal run scoring/standing requires the separate 7D hostile suite. |
