...
| `ENG-CONF-AUTHORITY-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7B implements immutable qualification plans/results, mechanically derived campaign authorization, outcome-independent attempt/replacement allocation with separate seed commitment, anchor/fresh-start prerequisites, invalidity, and governed decisions. Corrective Phase 7C/7D replay pins an Ed25519 verifier and grants a start only after authenticated canonical receipt bytes, typed fresh-start proof, configuration, qualification, and authenticated effective namespace-history resolution. | `ADR-004 R3`; `ADR-005 R2`; `ADR-011 R3`; `ADR-012 R3` | Contract, negative, replay, and regression tests over authority derivation, replacement seed closure, full typed authorization reconstruction, exact indexed bindings, signed receipt/start custody, namespace history, substituted-key and signed-payload attacks, and preflight forgery. | `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/formalAuthority.js`; `scn001_eval/src/formalEvidence.js`; `scn001_eval/src/formalRunRecord.js`; corresponding tests; Phase 7B-7D records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Synthetic tests exercise start and replay transitions, but no real effective namespace, external anchor event, campaign execution, or independent exact-commit review exists. |
...
| `ENG-CONF-SCORE-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7D implements the exact one/three-run branch, closed path/claim obligation matrix, nonnumeric conjunctive aggregation, hard-failure closure, indeterminacy, historical-failure preservation, bounded result/claim ceiling, immutable standing, supersession, and non-circular later-index authority. The corrective execution core now derives the five-class vector from actual four-path public-boundary execution instead of accepting caller outcome fields. | `ADR-004 R3`; `ADR-005 R2`; `ADR-009 R4`; `ADR-011 R3`; `ADR-012 R3` | Contract, negative, replay, and adversarial tests cover actual path execution, exact initialized history/resume attribution, temporal/comparison predicates, caller outcome rejection, missing-output derivation, non-pressure isolation, no voting/weighting, early stopping, independent recomputation, claim language, standing forks/terminality, durable supersession chains, and later namespace/receipt authority. | `scn001_eval/src/selectedSliceFormalRunner.js`; `scn001_eval/test/selectedSliceFormalRunner.test.js`; `scn001_eval/src/boundedScoring.js`; `scn001_eval/test/boundedScoring.test.js`; `scn001_eval/test_support/formalCampaignFixtures.js`; Phase 7D and corrective records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Derived outcomes remain an internal unsealed transcript until typed durable capture, external authority, formal-record sealing, and complete invalidity/hard-invariant review close; no real campaign result, owner standing, protected external receipt, or formal claim exists. |
...
trusted execution runner, and the four-path oracle transcript is not yet wired
to durable capture, invalidity derivation, interrupted-attempt retention, and
formal sealing. The four Phase 7 controls remain `applicable / uncovered`; the
