...
| `ENG-CONF-CAPTURE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public inspection capture includes proposal-intent state and contemporaneous candidate ancestry/evidence without becoming an alternate proposal-state source. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Closed ingress; passive cloned snapshot; proposal inspection/replay interleaving; negative non-reingestion test. Contract, negative, replay-limited modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_sut_core/test/run-state.test.js`; `scn001_eval/src/harness.js` | Default | uncovered | merge-blocking; claim-blocking | Proposal capture remains passive; no replay/restore implementation exists, and generic capture non-repair is tested only for current in-memory paths. |
...
| `ENG-CONF-DEP-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Current SUT transitions create source, basis, support, and proposal-to-candidate transition-ancestry relations consumed by inspection snapshots; proposal formation also preserves contemporaneous transition basis to the exact candidate. | `ADR-007 R3`; `ADR-008 R2` | Contemporaneous relation creation, exact candidate-transition result anchoring, proposal ancestry, proposal-transition candidate basis, no copied candidate evidence relations, exact comparison/candidate lineage, and passive inspection tests. Contract and negative-limited modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/test/run-state.test.js` | Default | uncovered | merge-blocking; claim-blocking | Proposal ancestry and transition basis are covered alongside prior exact-basis attacks; generic missing-lineage, backdated-reporting, post-hoc-basis, and later-snapshot attacks remain incomplete. |
...
| `ENG-CONF-HARNESS-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Evaluation harness delivery, run-local source-fact projection, output forwarding, and inspection capture exist; proposal intent formation adds no evaluation input, construction, selection, or transport behavior. | `ADR-008 R2` | Thin public-boundary transport; SUT-internal one-intent-per-current-candidate multiplicity; empty public outputs under one or multiple proposal intents; unchanged evaluation production code. Contract mode. | `scn001_eval/src/harness.js`; `scn001_eval/src/fixtureProjection.js`; `scn001_eval/test/harness.test.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/test/run-state.test.js` | Default | uncovered | merge-blocking; claim-blocking | Multiple internal proposal intents are covered without arbitration, but no SUT-selected realization output exists yet, so the required transport/non-arbitration trace remains absent. |
...
| `ENG-CONF-INSPECT-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public snapshots, record inspection, and relation enumeration now expose proposal material intent, exact candidate identity, copied scope, transition ancestry, transition basis, provenance, and ordering without mutation. | `ADR-006 R2`; `ADR-008 R2` | Clone-only inspection; candidate structural invariance; proposal ancestry/evidence inspection; before/after and replay tests. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/test/run-state.test.js` | Default | uncovered | merge-blocking; claim-blocking | Proposal-intent inspection is covered; surfaced realization, active-trial, outcome, and lifecycle-history inspection remain absent and require new coverage when introduced. |
...
| `ENG-CONF-SIM-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Simulator-realization fixture projection and SUT ingress roles already exist; internal proposal intent now exists but is not selected or emitted for realization. | `ADR-002 R2`; `ADR-005 R2`; `ADR-008 R2` | Shared closed ingress; simulator role/origin preservation; empty public output under proposal multiplicity; answer-bearing-field negative test; dependency scan. Contract and negative modes. | `scn001_eval/src/fixtureProjection.js`; `scn001_eval/test/harness.test.js`; `scn001_sut_core/src/inputValidation.js`; `scn001_sut_core/test/run-state.test.js`; `scripts/check-dependency-boundary.mjs` | Default | uncovered | merge-blocking; claim-blocking | No simulator implementation or proposal selected for realization exists, so realization non-arbitration is not yet exercised end to end. |
| `ENG-CONF-STATE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Proposal formation resolves only the immediately preceding current candidate-formation transition and its exact result refs; it does not search retained candidates, rank, or select first/latest/best/relevant state. | `ADR-004 R3`; `ADR-006 R2`; `ADR-007 R3` | Exact candidate-transition identity anchor; direct result resolution; candidate-identity proposal reuse/conflict checks; zero-match omission; multiple-proposal integrity failure; no retrieval/ranking API. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/test/run-state.test.js` | Default | uncovered | merge-blocking; claim-blocking | Current proposal access is selected-slice-specific and exact; realization selection, later-use applicability, and other state families remain unimplemented and require their own bounded access review. |
...
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Candidate-bound proposal-intent state, identity, replay, lineage, multiplicity, public inertness, and failure behavior changed with fail-without-it attacks. | Rule entry | Positive, negative, contract, regression, replay, ambiguity, equal-payload identity, structural immutability, contemporaneity, interleaving, family-boundary, multiplicity, and run-isolation tests cover proposal-intent formation. | `scn001_sut_core/test/run-state.test.js`; `scn001_eval/test/harness.test.js`; `scn001_sut_core/test/public-boundary.test.js`; `tests/conformance/`; `package.json` | Default | uncovered | merge-blocking; promotion-blocking | Current tests cover the bounded proposal-intent increment; future realization, binding, activation, and lifecycle behavior still require change-specific fail-without-it evidence. |
| `ENG-HEALTH-TEST-002` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Tests are used throughout this ledger as implementation and conformance evidence. | Rule entry | Public-boundary tests drive the positive proposal path, replay, equal-payload identity, multiplicity, interleaving, output inertness, and run isolation; direct `RunState` attacks cover candidate structural invariance and unreachable malformed/conflicting state only. | `scn001_sut_core/test/`; `scn001_eval/test/`; `tests/conformance/` | Default | uncovered | merge-blocking; claim-blocking | Test-validity review is not independently recorded; direct malformed/conflict and candidate-before/after attacks are implementation-only evidence, and no test is behavioral-compatibility evidence. |
