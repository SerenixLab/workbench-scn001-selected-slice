...
- **Verified at:** evidence resolved against workbench base commit `ac7fbb7ea48e0c3814337fc1674b1b6dfb4e549c`, canonical meta commit `a77a431ad1595a45db80c2f67a634971bebc7739`, the exact projected digests in `governance/ZOEY_GOVERNANCE.lock`, and this focused ledger correction. A fresh independent review must bind its result to the resulting exact correction commit.
...
| `ENG-AGENT-CODEX-003` | `R2` | `integrations/CODEX_INTEGRATION.md` | applicable | The lock, profile, integration release, source closure, and evaluation guidance changed for accepted ADR-010 through ADR-012. | `ENG-AGENT-001 R2` | Codex reread root and evaluation guidance after projection/instruction edits; a new qualifying human refresh review is not yet recorded. | `AGENTS.md`; `scn001_eval/AGENTS.md`; `governance/ZOEY_GOVERNANCE.lock`; `governance/integrations/CODEX_INTEGRATION.md` | Local refresh performed; human review pending. | uncovered | advisory or promotion-blocking by condition | The current task cannot self-attest the required human instruction refresh review. |
...
| `ENG-BASE-002` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Standard `V0.6.4`, profile `V0.5.0`, integration `V0.2.2`, source closure, seven revised/source-updated rules, and four new rules require explicit remapping. | Rule entry | Lock-to-index structural mapping; canonical `ADR-010-012-R3` revalidation audit. Contract mode. | `governance/ZOEY_GOVERNANCE.lock`; this complete ledger; canonical `engineering/revalidation/ADR-010-012-R3.md` introduced at semantic-source commit `288f694` and retained in projection commit `a77a431` | Automated mapping checks pass; qualifying human source-bound confirmation pending. | revalidation-required | claim-blocking | The current task cannot self-confirm the material canonical rule/profile remapping. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The completed implementation scope is the bounded engineering chain through `DP-OUTCOME-UPDATE` and `DP-EXPLAIN`, including the reviewed Phase 5–6 corrective and fixture-initialization hardening ranges. This correction has one separate purpose: make the current conformance ledger truthful without changing implementation behavior. | Rule entry | Historical implementation diffs and independent closures; focused ledger diff against base `ac7fbb7`; stale-basis, stale-scope, status, and artifact-label inventory; full local gate. | Current implementation/tests; review closures below; this focused `governance/CONFORMANCE.md` correction | Qualifying implementation reviews recorded below; the resulting ledger correction is routed to fresh independent review. | review-only | advisory or merge-blocking by condition | Implementation scope is independently reviewed. This ledger correction and the generated Phase 7 projection still require their stated fresh independent review; protected required-check configuration remains unverified. |
