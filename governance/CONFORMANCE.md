...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The current implementation increment performs only behavior-preserving internal SUT decomposition after the separately committed README truth synchronization. | Rule entry | Focused structural diff against documentation commit `e1e2126`; read-only closure/derivation extraction with unchanged `RunState` ownership, public boundary, and retained semantic contracts. | Current decomposition commit and diff; new private SUT modules; unchanged public-boundary and semantic regression tests; decomposition addendum below | Fresh independent review of both stabilization commits, including their pushed identities if later pushed, is pending; the implementing task does not self-attest a passing review. | uncovered | advisory or merge-blocking by condition | The bounded documentation and decomposition commits have not yet received qualifying independent review. Historical passing and failed reviews remain scoped to their recorded commits. |
...
| `ENG-HEALTH-STRUCTURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The SUT now separates retained-state closure, proposal closure, production activation derivation, and run-state orchestration into bounded private modules. | Rule entry | Each new module has one stateable responsibility and excludes state ownership/mutation; the split is justified by repeated concrete semantic pressure, review locality, testability, and preservation of accepted exact closure. | `README.md`; `scn001_sut_core/AGENTS.md`; `scn001_sut_core/src/`; package manifests and tests | Fresh independent review pending | uncovered | advisory or merge-blocking by condition | Module responsibilities are recorded, but fresh independent structure review is pending; future shared/top-level areas still require an index and guidance update. |
