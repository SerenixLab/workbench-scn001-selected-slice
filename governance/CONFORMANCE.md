...
The workbench contains a two-package boundary spine and a stabilized semantic segment. The SUT derives attributed current-user assertions, selected temporal eligibility, scoped recognition/spontaneous-production comparisons, immutable non-active trial candidates, distinct candidate-bound proposal intents, proposal-response binding, nine-check activation assessments, separate active trials, scoped focused-drill instructions and dispositions, exact behavior-realization closure, separate short-term focused-drill outcomes, and non-activation dispositions from raw run-local source facts and exact typed local relation closure.
It does not create current-skill facts, direct-correction behavior, delayed-correction trials, later-use applicability or behavior, later outcomes, explanations, or formal evaluation artifacts.
...
| `ENG-CONF-CAPTURE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public inspection passively exposes proposal/activation closure plus focused-drill instruction, disposition, realization, outcome, provenance, typed relations, and sole creating transitions. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Clone-only snapshots across both formal checkpoints, replay, malformed-closure attacks, and outcome participant inspection. Contract, negative, replay-limited modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/focusedDrill.js`; `scn001_sut_core/test/run-state.test.js`; `scn001_eval/src/harness.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | Current in-memory focused-drill capture is passive for the bounded family; no replay/restore implementation or formal evaluation capture exists. |
...
| `ENG-CONF-DEP-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Proposal, activation, focused-drill instruction/disposition, realization, and short-term outcome require exact contemporaneous transitions, unique creators, ingestion provenance, and typed participant relations. | `ADR-007 R3`; `ADR-008 R2` | Exact candidate ancestry; focused instruction source/basis; disposition basis; realization and outcome relations; sole-creator, participant-removal, substitution, and replay attacks. Contract, negative, and replay modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/proposalClosure.js`; `scn001_sut_core/src/productionTrialActivation.js`; `scn001_sut_core/src/focusedDrill.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | The bounded focused-drill lineage is covered; generic future-family lineage and lifecycle attacks remain incomplete. |
...
| `ENG-CONF-HARNESS-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | The formal one-argument harness transports one exact SUT-selected proposal or focused-drill request and gates D-001/D-002 and D-003/D-004 through passive exact checkpoints. | `ADR-008 R2` | Zero/one/multiple output attacks; exact CP-PROD-ACTIVE and CP-DRILL-REALIZATION-MATCH closure; staged fixture-set validation; mismatch non-delivery; override rejection. Contract and negative modes. | `scn001_eval/src/harness.js`; `scn001_eval/src/simulator.js`; `scn001_eval/src/simulatorProjection.js`; `scn001_eval/index.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | Non-arbitration and governed transport are covered through the focused-drill seam; direct correction and later simulator families require separate contracts. |
...
| `ENG-CONF-INSPECT-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Inspection passively exposes proposal/activation and focused-drill instruction/disposition/realization/outcome closure with exact typed relations and creators. | `ADR-006 R2`; `ADR-008 R2` | Clone-only inspection; emission invariance; checkpoint inspection; canonical claimant counts; malformed focused participant attacks never repair state. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/focusedDrill.js`; `scn001_sut_core/src/publicBoundary.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Bounded focused-drill inspection is covered; direct-correction and later lifecycle history remain absent. |
...
| `ENG-CONF-PAYLOAD-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Closed proposal and focused-drill outputs emit only from intact retained state; projectors validate complete role-specific simulator records before narrow exact-key ingress. | `ADR-005 R2`; `ADR-008 R2` | Exact output/full-record keys, opaque refs, requested/realized behavior and fidelity consistency, evaluation-only stripping, and full-record ingress rejection. Contract and negative modes. | `scn001_sut_core/src/inputValidation.js`; `scn001_sut_core/src/runState.js`; `scn001_eval/src/simulator.js`; `scn001_eval/src/simulatorProjection.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Current proposal and focused-drill shapes are closed; future simulator roles require equivalent negatives. |
...
| `ENG-CONF-PUBLIC-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | The unchanged SUT method set emits one closed proposal or focused-drill request only from an exact valid retained closure with no realization already recorded. | `ADR-008 R2`; `ENG-HEALTH-API-001 R2` | Frozen unchanged method object; exact output variants; malformed/competing closure attacks; no selector or semantic command. | `scn001_sut_core/index.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; private closure modules; SUT tests | Default | uncovered | merge-blocking; claim-blocking | Output cardinality and payload are covered; semantic answer-selector review remains manual for future outputs. |
...
| `ENG-CONF-RUN-001` | `R3` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public run lifecycle keeps source, activation, focused-drill instruction/disposition/realization, and short-term outcome identities isolated across independent runs. | `ADR-003 R2`; `ADR-004 R3`; `ADR-008 R2` | Independent runs create distinct focused-drill refs; foreign refs are absent and unresolvable; exact replay reuses same-run semantic closure. Negative and replay modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_eval/src/harness.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Focused-drill isolation is explicit; direct correction and later-use state remain absent. |
...
| `ENG-CONF-STATE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Proposal, activation, focused-drill output, realization, and outcome resolve exact retained identities without ranking, first/latest selection, or loose relation predicates. | `ADR-004 R3`; `ADR-006 R2`; `ADR-007 R3` | Exact proposal and focused-drill closure; distinct equal-payload instruction attacks; competing output/realization failure; complete outcome participant basis. Contract and negative modes. | `scn001_sut_core/src/runState.js`; private closure modules; `scn001_sut_core/test/run-state.test.js` | Default | uncovered | merge-blocking; claim-blocking | Bounded focused-drill resolution is narrow and exact; later-use applicability remains unimplemented. |
| `ENG-CONF-STATE-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Focused-drill instruction, disposition, realization closure, and outcome consume exact prior state and retain attributable transitions while `RunState` alone allocates and commits. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Exact key/input/result/order/role assertions; duplicate creator, missing participant, wrong target, no-mutation, replay, and provenance attacks. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/focusedDrill.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Focused-drill mutation is covered; direct correction, later-use applicability, and later lifecycle mutation remain absent. |
| `ENG-HEALTH-ABSTRACTION-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Private retained-state, proposal, production-activation, and focused-drill modules are non-throwaway internal abstractions under the SUT owner. | `ADR-001 R1` | `focusedDrill.js` is read-only derivation/validation justified by repeated concrete semantic pressure, review locality, testability, and preservation of accepted exact closure; mutation remains in `RunState`. | `scn001_sut_core/src/focusedDrill.js`; existing private modules; `scn001_sut_core/src/runState.js`; package tests; focused-drill addendum below | Fresh independent abstraction review of the focused-drill commit is pending; the historical decomposition review remains preserved below. | uncovered | advisory or merge-blocking by condition | The new bounded module has implementing-task review only; fresh qualifying independent review is required. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The current change has one primary purpose: implement the bounded DP-FOCUSED-DRILL trajectory after exact CP-PROD-ACTIVE. | Rule entry | Focused diff against starting `HEAD` `1f80bb55d58c8c68e0de103f4629713a83a7eec8`; formal harness path; adversarial SUT/evaluation tests; hostile implementing-task review. | Current focused-drill commit/diff; README; SUT/evaluation implementation and tests; addendum below | Fresh independent review of the pushed focused-drill commit is pending; historical review records remain scoped to their exact commits. | uncovered | advisory or merge-blocking by condition | The implementing task cannot independently close its own change-review condition. |
...
| `ENG-HEALTH-FAILURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Malformed proposal, activation, focused-drill participant, realization, outcome, provenance, target, and competing creator states fail closed without semantic repair. | Rule entry | Typed integrity errors; exact-key/reference errors; wrong actor/target, mismatch, missing participant, duplicate creator/trial, pre-mutation, and replay attacks. | SUT/evaluation source and package tests; repository gates | Default | uncovered | merge-blocking; claim-blocking | Failure transparency is covered for the bounded focused-drill seam; no operational reporting layer exists. |
...
| `ENG-HEALTH-STRUCTURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The SUT separates retained-state, proposal, production-activation, focused-drill derivation/validation, and run-state orchestration into bounded private modules. | Rule entry | `focusedDrill.js` derives and validates instruction/disposition/realization/outcome participant structures; mutable state, allocation, commit, lifecycle, public API, fixture gating, and later families do not belong there. | `README.md`; `scn001_sut_core/src/focusedDrill.js`; `scn001_sut_core/src/runState.js`; package tests; addendum below | Fresh independent structure review of the focused-drill commit is pending; prior decomposition review remains historical. | uncovered | advisory or merge-blocking by condition | The new module boundary has only implementing-task review and requires fresh independent review. |
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Regressions cover both focused-drill checkpoints, five-object separation, exact creators/participants, mismatch, pre-mutation failure, replay, run isolation, and denied later families. | Rule entry | Positive formal-harness full trajectory plus negative missing/wrong/duplicate actor, trial, instruction, output, target, realization, observation, feedback, and closure attacks. | SUT/evaluation tests; conformance tests; `package.json` | Default | uncovered | merge-blocking; promotion-blocking | The bounded focused-drill family is covered; direct correction and every later family remain future-triggered. |
| `ENG-HEALTH-TEST-002` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The canonical focused-drill trajectory uses the formal harness and public boundary; direct `RunState` use is limited to malformed retained-state, replay, isolation, and unreachable pre-mutation attacks. | Rule entry | End-to-end checkpoint delivery and simulator routing; raw fixture inputs contain no SUT conclusions; direct tests attack exact internal closure without becoming behavior evidence. | `scn001_sut_core/test/`; `scn001_eval/test/`; `tests/conformance/` | Default | uncovered | merge-blocking; claim-blocking | Fresh independent test-validity review is pending; tests remain engineering/conformance evidence only. |
