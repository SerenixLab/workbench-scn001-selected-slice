...
The workbench contains a two-package boundary spine and a stabilized semantic segment. The SUT derives attributed current-user assertions, selected temporal eligibility, scoped recognition/spontaneous-production comparisons, immutable non-active trial candidates, distinct candidate-bound proposal intents, proposal-response binding, nine-check activation assessments, separate active trials, and non-activation dispositions from raw run-local source facts and exact typed local relation closure.
It does not create current-skill facts, focused-drill or later-use behavior, outcomes, explanations, or formal evaluation artifacts.
...
| `ENG-CONF-CAPTURE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public inspection captures proposal, realization, binding, complete activation-control basis, role-qualified provenance, assessment, active trial, and exact creating-transition identity without driving or repairing them. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Passive cloned snapshots across formal routing/activation and replay; exact material-input, dual-role relation, and sole-creator assertions. Contract, negative, replay-limited modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_sut_core/test/run-state.test.js`; `scn001_eval/src/harness.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | Current in-memory activation capture is passive and complete for the bounded family; no replay/restore implementation or formal evaluation capture exists. |
...
| `ENG-CONF-DEP-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Proposal, binding, activation, and active-trial ancestry require exact contemporaneous typed transition, unique creating-transition, basis, ingestion, control-provenance, and relation evidence. | `ADR-007 R3`; `ADR-008 R2` | Exact candidate ancestry; complete activation-control inventory; role-qualified redelivery; exact ingestion basis; retained-state sole-creator resolution; clone/pointer-rewrite/partial-claim attacks; causal order and ancestry. Contract, negative, and replay modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/proposalClosure.js`; `scn001_sut_core/src/productionTrialActivation.js`; `scn001_sut_core/test/run-state.test.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | The bounded production-activation lineage is covered; generic future-family lineage and lifecycle attacks remain incomplete. |
...
| `ENG-CONF-INSPECT-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Inspection passively exposes proposal/realization/binding closure plus exact activation controls, role-qualified relations, assessment/trial state, and their sole creating transitions. | `ADR-006 R2`; `ADR-008 R2` | Clone-only inspection; emission invariance; canonical claimant counts; malformed-control/provenance/trigger/competing-transition attacks never repair state. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/proposalClosure.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/test/run-state.test.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | Bounded activation inspection is covered; outcome and later lifecycle history remain absent. |
...
| `ENG-CONF-PAYLOAD-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Boundary and retained-state validation complete before outward emission, simulator invocation, projection, activation assessment, or active-trial mutation. | `ADR-005 R2`; `ADR-006 R2`; `ADR-008 R2` | Pre-emission closure; atomic ingress; unknown-control, exact-trigger, and competing-creator no-mutation snapshots; malformed state fails before behavior or activation mutation. Negative and contract modes. | `scn001_sut_core/src/inputValidation.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/proposalClosure.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | No deserializer or telemetry path exists; future rejection paths need equivalent atomicity checks. |
| `ENG-CONF-PUBLIC-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | The unchanged SUT method set emits only from an exact valid selection, candidate-bound proposal closure, and zero valid realization closures. | `ADR-008 R2`; `ENG-HEALTH-API-001 R2` | Frozen unchanged method object; exact output keys; malformed proposal/realization attacks; no selector or transition command. | `scn001_sut_core/index.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/proposalClosure.js`; SUT tests | Default | uncovered | merge-blocking; claim-blocking | Output cardinality and payload are covered; semantic answer-selector review remains manual for future outputs. |
| `ENG-CONF-REF-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Proposal ancestry is the sole activation-candidate authority; controls use exact fact identities; assessment/trial creator pointers must agree with exactly one retained transition claimant. | `ADR-005 R2`; `ADR-007 R3`; `ADR-008 R2` | UUID-backed opaque refs; distinct-equal control, equal-scope candidate, fake/clone transition, creator-pointer rewrite, interaction-retargeting, rewritten ancestry, and cross-run attacks. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/proposalClosure.js`; `scn001_eval/src/simulatorProjection.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Candidate, fact, assessment, trial, transition, interaction, and realization identities are exact and run-scoped; later-use references remain future work. |
| `ENG-CONF-ROLE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Evaluation adapters preserve declared roles, and activation replay requires exact semantic-source actors plus separate candidate/binding logical roles for every control identity. | `ADR-005 R2`; `ADR-006 R2` | Full fixture/simulator source validation; wrong-actor attacks; region-qualified control relations; dual-role shared consequence/reversibility relation-removal attacks. | `scn001_eval/src/fixtureProjection.js`; `scn001_eval/src/simulatorProjection.js`; `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/productionTrialActivation.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Current activation roles, origins, actors, and identities are covered; future roles require equivalent attacks. |
| `ENG-CONF-RUN-001` | `R3` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public run lifecycle and closure-owned source, assessment, and active-trial identities remain isolated across independent formal runs. | `ADR-003 R2`; `ADR-004 R3`; `ADR-008 R2` | Independent formal runs create distinct activation-assessment and active-trial refs; ended-run refs are unresolvable and absent from the next run. Negative and replay modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_eval/src/harness.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | Activation and active-trial isolation is explicit; outcome state and later behavior remain absent. |
...
| `ENG-CONF-STATE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Selection, binding, activation, and output/trial availability resolve exact retained identities without ranking, arbitration, first/latest control selection, or loose relation predicates. | `ADR-004 R3`; `ADR-006 R2`; `ADR-007 R3` | Exact proposal/selection/realization resolution plus complete co-applicable control inventory, identity-based conflict/redelivery, material-context basis, and malformed closure attacks. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/proposalClosure.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/productionTrialActivation.js`; `scn001_sut_core/test/run-state.test.js` | Default | uncovered | merge-blocking; claim-blocking | Bounded activation resolution is narrow and exact; later-use applicability and other state families remain unimplemented. |
| `ENG-CONF-STATE-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Activation and active-trial creation consume one exact lineage/control chain and require exactly one retained creating transition for binding, assessment, and trial before reuse or mutation. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Exact transition/ref/order/role assertions; clone, pointer-rewrite, malformed/partial claimant, fake-trigger, no-mutation, substitution, provenance, replay, and ancestry attacks. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/proposalClosure.js`; `scn001_sut_core/src/productionTrialActivation.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | The bounded production activation mutation is covered; later-use applicability, behavior, outcome, and later lifecycle mutations remain absent. |
| `ENG-HEALTH-ABSTRACTION-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Private retained-state, proposal-closure, and production-activation modules are non-throwaway internal abstractions under the SUT owner. | `ADR-001 R1` | Read-only helpers justified by repeated concrete semantic pressure, review locality, testability, and preservation of accepted exact closure; `RunState` retains sole mutation ownership. | `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/proposalClosure.js`; `scn001_sut_core/src/productionTrialActivation.js`; `scn001_sut_core/src/runState.js`; package tests; independent-review closure below | Passing independent ChatGPT review of decomposition commit `187528d3375aee610b08c2c5b7587e44124762a5` is recorded below. | review-only | advisory or merge-blocking by condition | The current bounded abstractions have qualifying independent review; future generalization, new shared ownership, or broader abstraction requires fresh current-pressure justification and review. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The stabilization work consists of one documentation-truth commit and one separately focused behavior-preserving internal-decomposition commit. | Rule entry | Separate focused diffs against starting `HEAD` `15824900c2c9c64ef513d3d2229931fcaf166bff`; README synchronization commit `e1e21260382f5d8865947e0f0790126393dd18b8`; decomposition commit `187528d3375aee610b08c2c5b7587e44124762a5`; unchanged semantic and public-boundary contracts. | Both stabilization commits and diffs; README; private SUT modules; existing regression tests; independent-review closure below | Passing independent ChatGPT review of both stabilization commits is recorded below; prior historical reviews remain scoped to their recorded changes. | review-only | advisory or merge-blocking by condition | The two stabilization changes received qualifying independent review. The next focused-drill semantic increment is a new non-throwaway change and requires fresh change-specific review. |
...
| `ENG-HEALTH-FAILURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Malformed proposal, realization, activation control, provenance, trigger, reference, and competing creating-transition states fail closed rather than mutating state or satisfying replay. | Rule entry | Typed integrity errors; exact-key/type/reference errors; clone, pointer-rewrite, partial/wrong-kind claimant, fake-trigger, wrong-interaction, no-mutation, and negative replay attacks. | SUT/evaluation source and package tests; repository gates | Default | uncovered | merge-blocking; claim-blocking | Failure transparency is covered for the bounded seam; no operational reporting layer exists. |
...
| `ENG-HEALTH-STRUCTURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The SUT separates retained-state closure, proposal closure, production-activation derivation, and run-state orchestration into bounded private modules. | Rule entry | Each module has one stated responsibility and excludes state ownership or mutation; the split is justified by repeated concrete semantic pressure, review locality, testability, and preservation of accepted exact closure. | `README.md`; `scn001_sut_core/AGENTS.md`; `scn001_sut_core/src/`; package manifests and tests; independent-review closure below | Passing independent ChatGPT structure review of decomposition commit `187528d3375aee610b08c2c5b7587e44124762a5` is recorded below. | review-only | advisory or merge-blocking by condition | The current module responsibilities received qualifying review; new top-level areas, shared ownership, or materially widened module responsibilities require fresh review. |
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Regressions cover complete activation controls/provenance plus exact assessment/trial creator uniqueness, malformed claimants, pre-mutation failure, replay, and run isolation. | Rule entry | Positive formal-harness sole-creator/replay assertions plus negative clone, pointer-rewrite, partial/wrong-field claimant, fake-trigger, exact-interaction, no-mutation, basis, provenance, and non-scope modes. | SUT/evaluation tests; conformance tests; `package.json` | Default | uncovered | merge-blocking; promotion-blocking | The bounded activation closure is covered; later use, behavior, outcome, and formal evaluation remain future-triggered. |
| `ENG-HEALTH-TEST-002` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The canonical activation/replay path uses the formal harness and declared SUT boundary; direct `RunState` use is limited to unreachable malformed retained-transition and pre-mutation attacks. | Rule entry | End-to-end formal harness creation/replay/sole-creator checks; package-root API tests; direct clone, pointer-rewrite, malformed claimant, and no-mutation integrity tests. | `scn001_sut_core/test/`; `scn001_eval/test/`; `tests/conformance/` | Default | uncovered | merge-blocking; claim-blocking | Fresh independent test-validity review is pending; these remain engineering tests, not formal behavioral evidence. |
