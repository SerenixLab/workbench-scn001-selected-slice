...
This ledger is the complete applicability router for the 53 rules pinned by
`governance/ZOEY_GOVERNANCE.lock`. The current projection is from clean
canonical `zoey-meta` commit `a77a431ad1595a45db80c2f67a634971bebc7739`
and includes standard `V0.6.4`, selected-slice profile `V0.5.0`, Codex
integration `V0.2.2`, `OPEN_QUESTIONS.md V0.2.23`, and accepted `ADR-010 R3`,
`ADR-011 R3`, and `ADR-012 R3`. Canonical commit
`288f694a29554ef8bddc6fe1d5d51a7ee553266e` is the earlier semantic
revalidation-source commit and direct ancestor of the current projection; it is
not the current projected product. Earlier ADR-009 and implementation
verification bases are preserved below as historical records rather than
rewritten as current review evidence.

Later change-specific addenda and independent-review closures record subsequent selected-slice semantic increments without rewriting that historical verification basis. Commit identities, rule counts, version transitions, review states, and implementation scopes in those explicitly historical sections describe their recorded moment and do not override this current authoritative table. The projected lock and snapshot digests identify the exact canonical governance basis; each change-specific record identifies its own starting commit, scope, and review disposition.
...
- **Verified at:** evidence resolved against exact reviewed workbench commit `2385020961e66c64edbf332ec07b3f7ff809b089`, canonical meta commit `a77a431ad1595a45db80c2f67a634971bebc7739`, the exact projected digests in `governance/ZOEY_GOVERNANCE.lock`, the qualifying independent `PASS`, and the project-owner approval recorded in the dated closure below.
...
| `ENG-AGENT-CODEX-003` | `R2` | `integrations/CODEX_INTEGRATION.md` | applicable | The lock, profile, integration release, source closure, and evaluation guidance changed for accepted ADR-010 through ADR-012. | `ENG-AGENT-001 R2` | Codex reread root and evaluation guidance after projection/instruction edits; the project owner approved the refreshed instruction chain on 2026-07-21. | `AGENTS.md`; `scn001_eval/AGENTS.md`; `governance/ZOEY_GOVERNANCE.lock`; `governance/integrations/CODEX_INTEGRATION.md`; Phase 7 source-bound owner-approval closure below | Local refresh and qualifying human review recorded below. | review-only | advisory or promotion-blocking by condition | The approved refresh closes this baseline only; later instruction, lock-baseline, profile, integration, or routing changes require another refresh decision before unrelated governed implementation continues. |
...
| `ENG-BASE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | All implementation, tests, docs, and governance artifacts depend on the accepted evaluation decision set. | `OPEN_QUESTIONS.md V0.2.23`; pinned ADR snapshots through `ADR-012 R3` | Local pinned sources; governance digest audit; source-authority comparison against clean canonical meta. Contract mode. | `governance/ZOEY_GOVERNANCE.lock`; `governance/sources/`; `scripts/check-governance-lock.mjs`; `README.md` | Automated projection checks plus source review; required CI integration remains unverified. | uncovered | promotion-blocking; claim-blocking | Projection integrity and source closure resolve; protected required-check configuration remains unverified. |
| `ENG-BASE-002` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Standard `V0.6.4`, profile `V0.5.0`, integration `V0.2.2`, source closure, seven revised/source-updated rules, and four new rules require explicit remapping. | Rule entry | Lock-to-index structural mapping; canonical `ADR-010-012-R3` revalidation audit. Contract mode. | `governance/ZOEY_GOVERNANCE.lock`; this complete ledger; canonical `engineering/revalidation/ADR-010-012-R3.md` introduced at semantic-source commit `288f694` and retained in projection commit `a77a431`; qualifying independent review and owner approval below | Automated mapping checks and source-bound owner confirmation recorded; protected required-check configuration remains unverified. | uncovered | claim-blocking | The material remapping is approved, but `CI-required` promotion integration and protected required-check configuration remain unverified. Future baseline changes require new rule-level revalidation. |
| `ENG-BASE-CONFORMANCE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | This ledger is being audited and cites gates, evidence paths, revisions, and statuses. | Rule entry | Evidence-path review; static conformance audit; aggregate local gate. Contract mode. | `governance/CONFORMANCE.md`; `governance/zoey_governance.py`; `scripts/run-gates.mjs`; `.github/workflows/ci.yml` | Default | uncovered | promotion-blocking; claim-blocking | The checker validates complete coverage, duplicate rows, exact lock revision/source mapping, allowed applicability/status values, and `not-applicable`/`N/A` consistency. Evidence-path existence, gate-name resolution, actual promotion integration, and semantic status sufficiency remain manual. |
| `ENG-BASE-CONFORMANCE-002` | `R1` | `ENGINEERING_STANDARD.md` | applicable | The active governance set changed and this index must disposition all 53 locked rules exactly once. | Canonical governance lock | Complete table; exact lock coverage, duplicate, revision/source, applicability, and status audit; repository-specific rationales. Contract mode. | `governance/ZOEY_GOVERNANCE.lock`; this ledger; `governance/zoey_governance.py` | Default | uncovered | promotion-blocking; claim-blocking | Mechanical exactness is covered; truthful applicability, evidence meaning, and future-trigger remapping still require review. |
...
| `ENG-BASE-PROFILE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The active selected-slice profile is `V0.5.0`; it is the only active profile. | Rule entry | Lock structure and digest validation; profile rule-ID uniqueness; manual composition review. Contract mode. | `governance/ZOEY_GOVERNANCE.lock`; `governance/profiles/SCN001_SELECTED_SLICE.md`; `governance/zoey_governance.py`; `scripts/check-governance-lock.mjs`; owner-approval closure below | Automated structural checks and approved semantic remapping; protected required-check configuration remains unverified. | uncovered | promotion-blocking; claim-blocking | Structural composition resolves, there is no second active profile, and the `V0.5.0` remapping is approved; protected required-check configuration remains unverified. |
| `ENG-BASE-PUBLISH-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | This governed workbench consumes a pinned local projection synchronized transactionally from clean canonical meta commit `a77a431ad1595a45db80c2f67a634971bebc7739`. | Rule entry | Local snapshot/digest validation, governing-source closure, and completeness audit. Contract mode. | `governance/`; `governance/ZOEY_GOVERNANCE.lock`; `governance/zoey_governance.py`; `scripts/check-governance-lock.mjs`; qualifying independent review recorded below | Default | uncovered | promotion-blocking; claim-blocking | Projection integrity, canonical worktree cleanliness, and independent generated-artifact review resolve; protected required-check verification remains open. |
| `ENG-BASE-REPO-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The repository remains a governed scenario-provisional workbench; `REPO-001` is unchanged by the accepted evaluation decisions. | `OPEN_QUESTIONS.md V0.2.23` | Structural declarations; governance/boundary checks; source comparison. | `README.md`; `AGENTS.md`; `governance/ZOEY_GOVERNANCE.lock`; `tests/conformance/boundary.test.js`; owner-approval closure below | Qualifying project-owner source-bound review recorded below. | review-only | promotion-blocking; claim-blocking | No durable-project claim is introduced; extraction into a durable project still triggers `REPO-001` and requires its own accepted decision. |
| `ENG-CHANGE-001` | `R5` | `ENGINEERING_STANDARD.md` | applicable | The evaluation questions are resolved; Phase 7 must comply with ADR-010/011/012 while unresolved maintenance, repository, memory, growth, trust, product, surface, continuity, and legacy triggers remain active boundaries. | `OPEN_QUESTIONS.md V0.2.23`; `ADR-009 R4`; `ADR-010 R3`; `ADR-011 R3`; `ADR-012 R3` | README frontier; pinned sources; accepted-contract and unresolved-trigger audit. | `README.md`; `AGENTS.md`; `scn001_eval/AGENTS.md`; `governance/sources/`; `governance/CONFORMANCE.md`; owner-approval closure below | Qualifying project-owner source-bound review recorded below. | review-only | promotion-blocking; claim-blocking | Trigger/compliance review remains manual; Phase 7 must follow ADR-010 through ADR-012 and all unresolved frontier triggers remain active. |
| `ENG-CLAIM-001` | `R5` | `ENGINEERING_STANDARD.md` | applicable | README, guidance, tests, ledger, and review language distinguish development evidence, formal authority, bounded results, and completion under the accepted contracts. | `OPEN_QUESTIONS.md V0.2.23`; `ADR-009 R4`; `ADR-010 R3`; `ADR-011 R3`; `ADR-012 R3` | Bounded claim/authority review; governance and boundary tests. Contract mode. | `README.md`; `governance/CONFORMANCE.md`; `governance/sources/`; `tests/conformance/boundary.test.js`; owner-approval closure below | Source-bound owner review recorded; required CI/protected-check integration remains unverified. | uncovered | claim-blocking; promotion-blocking | No dedicated semantic claim-language checker or verified protected required check exists. |
| `ENG-CLAIM-002` | `R5` | `ENGINEERING_STANDARD.md` | applicable | Repository artifacts explicitly deny retroactive promotion; Phase 7 code defines prospective formal-artifact mechanisms, but all constructed test objects are synthetic/non-formal and no accepted campaign artifact exists. | `OPEN_QUESTIONS.md V0.2.23`; `ADR-004 R3`; `ADR-005 R2`; `ADR-009 R4`; `ADR-010 R3`; `ADR-011 R3`; `ADR-012 R3` | Formal-label/retroactive-promotion inventory, typed authority reconstruction, and `P -> D -> A` boundary review. Contract and negative modes. | `README.md`; `AGENTS.md`; `scn001_eval/AGENTS.md`; Phase 7 source/tests; `governance/CONFORMANCE.md`; `governance/sources/` | Source-bound owner review recorded for the baseline; fresh Phase 7 exact-commit review and protected integration remain pending. | uncovered | claim-blocking; promotion-blocking | No automated reserved-artifact/authority scan exists; mechanism tests cannot attest prospective external events or become formal evidence. |
| `ENG-CLAIM-WORKBENCH-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | This remains the first selected-slice scenario-provisional workbench; placement, denied claims, and `REPO-001` trigger are unchanged. | `ADR-001 R1`; `ADR-008 R2`; `OPEN_QUESTIONS.md V0.2.23`; `ENG-BASE-REPO-001 R2` | Structural declarations; source comparison. Contract mode. | `README.md`; `AGENTS.md`; `governance/ZOEY_GOVERNANCE.lock`; `governance/CONFORMANCE.md`; owner-approval closure below | Qualifying project-owner source-bound review recorded below. | review-only | promotion-blocking; claim-blocking | Workbench role remains exact; extraction still triggers `REPO-001`, and no durable system-project or broader architecture claim is approved. |
| `ENG-CONF-AUTHORITY-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7B implements immutable qualification plans/results, mechanically derived campaign authorization, outcome-independent attempt/replacement allocation with separate seed commitment, anchor/fresh-start prerequisites, invalidity, and governed decisions. Corrective Phase 7C/7D replay pins an Ed25519 verifier and grants a start only after authenticated canonical receipt bytes, typed fresh-start proof, configuration, qualification, and authenticated effective namespace-history resolution. The internal attempt pipeline additionally requires the signed start to bind its actual recorder-owned run scope and exact primary/replacement allocation before execution. | `ADR-004 R3`; `ADR-005 R2`; `ADR-011 R3`; `ADR-012 R3` | Contract, negative, replay, and regression tests over authority derivation, replacement seed closure, full typed authorization reconstruction, exact indexed bindings, signed receipt/start custody, namespace history, substituted-key, signed-payload/run-scope/allocation attacks, invalidity evidence replay, replacement execution/limits, and preflight forgery. | `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/formalAuthority.js`; `scn001_eval/src/formalEvidence.js`; `scn001_eval/src/formalRunRecord.js`; `scn001_eval/src/selectedSliceFormalPipeline.js`; `scn001_eval/src/boundedCampaignRunner.js`; corresponding tests; Phase 7B-7D and corrective records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Synthetic tests close sealable invalidity-to-replacement execution and accepted stop limits. Owner-governed qualified-contingency/supersession continuation is not a resumable end-to-end path; no real effective namespace, external anchor event, or independent exact-commit review exists. |
| `ENG-CONF-CAPTURE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public inspection remains passive and run-local; the separate Phase 7C evaluator-owned capture mechanism seals exact supplied bytes and typed semantics without mutating, repairing, selecting, or feeding evidence back into SUT state. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2`; `ADR-011 R3` | Existing clone/checkpoint attacks plus exact-byte, closed-envelope, evaluator-privacy, simulator request/realization, actor-separation, and durable replay tests. | Existing bounded SUT/evaluation capture modules; `scn001_eval/src/formalEvidence.js`; `scn001_eval/test/formalEvidence.test.js`; package tests | Default plus local-recorded/CI-required for formal-capture mechanisms; protected-check verification pending. | uncovered | merge-blocking; claim-blocking | Formal capture accepts already-observed material; it does not establish that a real campaign generated it, and no synthetic object may be promoted retroactively. |
| `ENG-CONF-CLAIM-001` | `R5` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | README and ledger deny current formal authority/results and bind future selected-slice claims to the four new profile controls. | `OPEN_QUESTIONS.md V0.2.23`; `ADR-004 R3`; `ADR-005 R2`; `ADR-009 R4`; `ADR-010 R3`; `ADR-011 R3`; `ADR-012 R3`; `ENG-CLAIM-001 R5`; `ENG-CONF-CONFIG-001 R1`; `ENG-CONF-AUTHORITY-001 R1`; `ENG-CONF-EVIDENCE-002 R1`; `ENG-CONF-SCORE-001 R1` | Bounded claim, authority, result-standing, and `P -> D -> A` review. Contract mode. | `README.md`; `governance/CONFORMANCE.md`; `governance/sources/`; owner-approval closure below | Source-bound owner review recorded; required CI/protected-check integration remains incomplete. | uncovered | claim-blocking | No dedicated formal-status/claim-ceiling checker exists and required promotion integration remains incomplete. |
| `ENG-CONF-CONFIG-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7A implements closed V1 behavior/evaluation manifest construction and validation, RFC8785-JCS canonicalization, domain-separated fingerprints, exact typed references/resolution, correction lineage, and fail-closed comparison. The trusted campaign entry measures a clean exact Git commit from the repository that loaded the evaluator, independently reconstructs both manifests, and permits only conservative three-run V1 authority. | `ADR-010 R3`; `ADR-011 R3` | Contract, negative, property, reconstruction, and regression tests over exact schemas, canonicalization, fingerprints, references, recursive source/runtime/policy closure, static imports, environment access, Git provenance, runtime-root substitution, private-layout independence, caller-invented manifest bypass, aliases, and tampering. | `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/configurationIdentity.js`; `scn001_eval/src/configurationMeasurement.js`; `scn001_eval/src/boundedCampaignRunner.js`; corresponding tests; Phase 7A and corrective records below | Local-recorded tests and full gate; repository CI runs the full gate, but hosted result and protected required-check configuration remain unverified for the corrective head. | uncovered | merge-blocking; promotion-blocking; claim-blocking | The trusted entry remains non-public and measured V1 deliberately disables qualification; generic constructors remain test mechanisms, no manifest/campaign has been published, and fresh independent exact-commit review remains required before formal reliance. |
| `ENG-CONF-DEP-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Every behavior-driving stage from proposal through explanation requires exact contemporaneous transitions, unique creators, original/current ingestion provenance, exact result occurrence, and typed participant relations. | `ADR-007 R3`; `ADR-008 R2` | Shared strict creator and retained-foundation validation; exact transition envelopes and source bindings; participant removal, duplication, substitution, retargeting, clone, replay, parity, and checkpoint attacks across the complete bounded segment. Contract, negative, and replay modes. | `scn001_sut_core/src/runState.js`; bounded SUT closure modules; evaluation checkpoint modules; package tests; `PHASE5_6_PARITY_INVENTORY.md` | Default | uncovered | merge-blocking; claim-blocking | The implemented lineage is covered through typed explanation support; generic future families and any future lifecycle kind require equivalent closure and attacks. |
| `ENG-CONF-DEP-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | The original and delayed formed/non-active candidates remain immutable historical endpoints while separate assessments, active trials, applicability records, and outcomes refer to exact effective versions. | `ADR-007 R3` | Append-only record identity; explicit lifecycle version/status; candidate-before/after regressions; copied immutable scope; delayed-candidate/active-trial separation and later-use endpoint checks. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/delayedCorrection.js`; `scn001_sut_core/src/laterUse.js`; package tests; `README.md` | Default | uncovered | merge-blocking; claim-blocking | No existing endpoint is mutated or superseded in place; any future supersession or retirement must create and test a new versioned record. |
| `ENG-CONF-EVIDENCE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | not-applicable | Current tests remain implementation/conformance evidence only; no artifact claims selected-slice behavioral compatibility or evaluated SUT behavior. Future trigger: first such trace, demo, report, or claim-supporting test. | `ADR-008 R2`; `ENG-CLAIM-001 R5` | Claim-boundary inventory and explicit README denial rechecked under the R5 dependency. | `README.md`; `governance/CONFORMANCE.md`; current tests | Not applicable until trigger; future rule requires local-recorded plus CI-required. | N/A | claim-blocking | Tests must be relabeled and reviewed if reused as behavior evidence; they can never be promoted retroactively into formal evidence. |
| `ENG-CONF-EVIDENCE-002` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7C/7D implement immutable closed evidence envelopes; exact raw-byte custody/replay; recorder-owned attempt chronology; pre-proof raw initial inspection; canonical typed control proofs; explicit attempted/withheld/delivered input distinctions; cumulative namespace/frozen campaign indexes; complete attempt inventories; durable start authority; and sealed formal run records with exact evidence, control, simulator, allocation, outcome, invalidity, and failure-finding closure. | `ADR-002 R2`; `ADR-008 R2`; `ADR-011 R3` | Contract, negative, replay, and regression tests cover bytes, envelopes, label-only proof rejection, caller-order rejection, privacy, simulator fidelity, authority reconstruction, namespace history/forks, attempt closure, run evidence partition/order, raw initial-snapshot reconstruction, run isolation, prospective selection, delivery interruption, withheld fixtures, SUT-failure attribution, orthogonal validity/invariant/obligation outcomes, and exact findings. | `scn001_eval/src/formalEvidence.js`; `scn001_eval/src/formalRunRecord.js`; corresponding tests/support; Phase 7C/7D records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Tests use synthetic objects only; post-start evidence/seal failures that cannot satisfy run integrity remain abandoned for authority review, no test artifact is promotable, and exact-commit independent review is pending. |
| `ENG-CONF-HARNESS-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | The development one-argument harness transports exactly one SUT-selected proposal or behavior request across proposal, focused, direct, and later-delayed paths; it reconstructs each required checkpoint and never chooses a semantic branch, timing, or winning output. | `ADR-008 R2` | Exact production, focused, direct, delayed-candidate/activation, later-use/realization, canonical-intervention, outcome, and explanation checkpoint reconstruction; zero/multiple output non-arbitration; immutable source binding; staged validation before ingress. Contract and negative modes. | `scn001_eval/src/harness.js`; `scn001_eval/src/productionActiveCheckpoint.js`; `scn001_eval/src/laterOutcomeCheckpoint.js`; `scn001_eval/src/sourceBindingLedger.js`; `scn001_eval/src/simulator.js`; `scn001_eval/src/simulatorProjection.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Non-arbitration and governed transport cover the completed engineering segment; the harness and its in-memory checkpoint state are development/conformance infrastructure, never formal evidence. |
...
| `ENG-CONF-INSPECT-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Inspection passively exposes the complete bounded state/transition/relation lineage through later outcome, uncertainty, explanation, and typed support without creating, repairing, or selecting semantic state. | `ADR-006 R2`; `ADR-008 R2` | Clone-only inspection; frozen original provenance; independent checkpoint comparison with immutable evaluation-owned bindings; malformed participants and terminal-family ambiguity fail without repair or staged ingress. Contract and negative modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; bounded SUT closure modules; evaluation checkpoint modules; package tests | Default | uncovered | merge-blocking; claim-blocking | Bounded inspection covers the completed segment; replay/restore and future lifecycle surfaces remain unimplemented and require new passivity review. |
| `ENG-CONF-INSPECT-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Inspection can interleave across ingestion, normalization, redelivery, exact-basis reuse, every selected-slice semantic stage, output routing, and terminal outcome/explanation processing. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Interleaving/replay equivalence, repeated passive checkpoint reconstruction, terminal-family/cardinality attacks, and transaction rollback checks; canonical redelivery cannot retarget first-ingestion identity. Interleaving, replay, negative modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; bounded SUT closure modules; evaluation checkpoint modules; package tests | Default | uncovered | merge-blocking; claim-blocking | No RNG, model, tool, provider, replay store, or lazy cache exists; each future surface requires expanded interleaving coverage. |
| `ENG-CONF-PAYLOAD-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Closed proposal, focused, direct, and later-delayed outputs emit only from intact retained state; fixture and simulator projectors validate complete role-specific records and strip evaluator-only canonical-premise material before narrow exact-key SUT ingress. | `ADR-005 R2`; `ADR-008 R2` | Exact output/full-record keys, opaque refs, requested/realized behavior and fidelity consistency, role preservation, evaluator-only field stripping, generic-delivery denial, and full-record ingress rejection. Contract and negative modes. | `scn001_sut_core/src/inputValidation.js`; `scn001_sut_core/src/runState.js`; `scn001_eval/src/fixtureProjection.js`; `scn001_eval/src/simulator.js`; `scn001_eval/src/simulatorProjection.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | All implemented simulator roles are closed; any future role or formal schema requires its own prospective contract and equivalent negatives. |
| `ENG-CONF-PAYLOAD-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Boundary, retained-state, checkpoint, and source-binding validation completes before fixture ingress, outward emission, simulator invocation, projection, or every implemented semantic mutation through explanation. | `ADR-005 R2`; `ADR-006 R2`; `ADR-008 R2` | Original projected meaning and immutable first-ingestion identity are reconstructed independently; evaluation bindings commit only after successful ingress and identity resolution; processing is transactional; failed ingress, closure, checkpoint, or source validation leaves no partial binding or derived semantic state. Negative and contract modes. | `scn001_sut_core/src/inputValidation.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; private closure modules; evaluation checkpoint modules; `scn001_eval/src/sourceBindingLedger.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Implemented rejection paths are atomic and rollback-tested; no deserializer, telemetry, durable replay, or formal capture path exists. |
| `ENG-CONF-PUBLIC-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | The unchanged SUT method set emits exactly one closed proposal, focused, direct-current-session, or later-delayed behavior request only from exact retained state with no matching realization already recorded. | `ADR-008 R2`; `ENG-HEALTH-API-001 R2` | Frozen unchanged method object; exact output variants and cardinality; malformed/competing closure attacks; no direct semantic command, selector, fixture ID, checkpoint, canonical-premise verdict, or oracle material. | `scn001_sut_core/index.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; private closure modules; SUT tests | Default | uncovered | merge-blocking; claim-blocking | Implemented output cardinality and payload are covered; semantic answer-selector review remains manual for future outputs. |
| `ENG-CONF-REF-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Exact opaque ancestry and creator references bind proposal, both activation chains, later applicability/realization, outcome, uncertainty, explanation, and support; controls and ingestion evidence must resolve one run-local identity and one retained transition claimant. | `ADR-005 R2`; `ADR-007 R3`; `ADR-008 R2` | UUID-backed refs; original interaction/ingestion bindings; source-to-fact bijection; exact meaning/creator/result closure; coordinated rewrite, rebound, duplicate, retarget, clone, and cross-run attacks. | `scn001_sut_core/src/runState.js`; bounded SUT closure modules; `scn001_eval/src/sourceBindingLedger.js`; evaluation checkpoint modules; package tests | Default | uncovered | merge-blocking; claim-blocking | Implemented references are exact and run-scoped through typed explanation support; future artifact/reference kinds require equivalent closure. |
| `ENG-CONF-ROLE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Evaluation adapters preserve declared roles and state origin across both activation chains, focused/direct/later realization, outcome, uncertainty, explanation, and support; checkpoint reconstruction requires exact semantic-source actors and logical roles. | `ADR-005 R2`; `ADR-006 R2` | Independent reconstruction of role/ref maps; full source and transition validation; wrong-actor, changed-role-same-target, region-qualified control, substituted attribution, and role-envelope attacks. | Evaluation projectors and checkpoint modules; `scn001_sut_core/src/runState.js`; bounded SUT closure modules; package tests | Default | uncovered | merge-blocking; claim-blocking | All implemented roles, origins, actors, and identities are covered; future roles require equivalent attacks. |
| `ENG-CONF-RUN-001` | `R3` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public run lifecycle keeps every source, ingestion, semantic record, output, realization, outcome, uncertainty, explanation, support, transition, and relation identity isolated across independent runs through the completed segment. | `ADR-003 R2`; `ADR-004 R3`; `ADR-008 R2` | SUT bindings and evaluation ledgers are per-run; exact development replay reuses validated closures; end-run clears state; fresh runs share no semantic or transport identity. Negative and replay modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_eval/src/harness.js`; `scn001_eval/src/sourceBindingLedger.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Isolation is explicit for all implemented families; no durable replay/restore or cross-run formal record authority exists. |
| `ENG-CONF-SCORE-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7D implements the exact one/three-run branch, closed path/claim obligation matrix, nonnumeric conjunctive aggregation, hard-failure closure, indeterminacy, historical-failure preservation, bounded result/claim ceiling, immutable standing, supersession, and non-circular later-index authority. The trusted measured path derives conservative attempts and bounded results, seals replayable invalidity, executes deterministic replacements, and suspends at accepted limits; qualified mechanism execution independently detects post-qualification divergence and halts before scoring. | `ADR-004 R3`; `ADR-005 R2`; `ADR-009 R4`; `ADR-011 R3`; `ADR-012 R3` | Contract, negative, replay, and adversarial tests cover actual path execution, caller outcome rejection, sealed findings/`NOT_REACHED`, invalidity evidence/decision/replacement closure, SUT-failure anti-retry attribution, qualification recomputation/divergence suspension, complete conservative campaigns, exact prior-history context, no voting/weighting, early stopping, standing forks/terminality, durable supersession chains, and later namespace/receipt authority. | `scn001_eval/src/selectedSliceFormalRunner.js`; `scn001_eval/src/selectedSliceFormalPipeline.js`; `scn001_eval/src/deterministicQualificationRunner.js`; `scn001_eval/src/boundedCampaignRunner.js`; corresponding tests; `scn001_eval/src/boundedScoring.js`; Phase 7D and corrective records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Measured conservative closure includes invalidity/replacement limits, but accepted owner-governed qualified-contingency continuation is not resumable; every execution is synthetic and no real standing, protected integration, independent exact-commit review, or formal claim exists. |
| `ENG-CONF-SIM-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | The deterministic simulator realizes exactly the SUT-requested proposal, focused immediate-correction, direct turn-completion, or later delayed-correction disposition; evaluator-private canonical-intervention fields remain outside SUT input. | `ADR-002 R2`; `ADR-005 R2`; `ADR-008 R2` | Exact request/full-record validation; renderers copy rather than choose timing or branch; fidelity/mismatch consistency; canonical-premise reconstruction remains evaluation-private; narrow projections. | `scn001_eval/src/simulator.js`; `scn001_eval/src/simulatorProjection.js`; `scn001_eval/src/harness.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | All implemented renderers are bounded and covered; no general simulator framework, formal campaign execution, or formal evidence exists. |
| `ENG-CONF-STATE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Every implemented proposal, activation, focused/direct/delayed disposition, realization, later applicability, outcome, uncertainty, and explanation operation resolves exact retained identities without retrieval, ranking, first/latest selection, or loose relation predicates. | `ADR-004 R3`; `ADR-006 R2`; `ADR-007 R3` | Exact closure modules; canonical first-ingestion identity; competing outputs and terminal families; retargeted, cloned, duplicated, and substituted evidence attacks. Contract and negative modes. | `scn001_sut_core/src/runState.js`; private closure modules; package tests | Default | uncovered | merge-blocking; claim-blocking | Bounded resolution is narrow and exact through explanation; future families must not introduce search or implicit selection. |
| `ENG-CONF-STATE-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | All implemented semantic records from focused behavior through delayed activation, later use, outcome, uncertainty, explanation, and support consume exact prior state and retain attributable transitions while `RunState` alone allocates and commits. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Exact key/input/result/order/role assertions; creator and ingestion closure; duplicate/missing/wrong participant attacks; full processing-transaction rollback; no-mutation, replay, provenance, and run-isolation checks. Contract and negative modes. | `scn001_sut_core/src/runState.js`; bounded SUT closure modules; package tests | Default | uncovered | merge-blocking; claim-blocking | Mutation and rollback are covered through the completed segment; any future lifecycle or persistence owner requires equivalent attribution and atomicity review. |
| `ENG-HEALTH-ABSTRACTION-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Existing bounded SUT/checkpoint abstractions remain unchanged; Phase 7 keeps configuration identity, prospective authority, durable evidence/run custody, and bounded scoring/standing in explicit evaluator-owned layers. | `ADR-001 R1` | Responsibility review plus dependency and hostile suites verify exact authority resolution without private SUT imports or SUT mutation, and scoring consumes sealed records rather than capture internals. | Existing bounded modules; Phase 7 evaluator modules/tests | Implementing-task responsibility review; fresh independent Phase 7 structure review pending. | uncovered | advisory or merge-blocking by condition | The bounded evaluator architecture is locally justified but not independently reviewed and is not claimed as permanent evaluation architecture. |
| `ENG-HEALTH-API-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The SUT public surface remains unchanged; the evaluation package root exports only the development one-argument harness constructor, with renderer/projector helpers and failure injection internal. | Rule entry | Exact SUT and evaluation package-root export tests; constructor override rejection; internal-only SUT resolvers and evaluation mechanism-test seam. | `scn001_sut_core/index.js`; `scn001_eval/index.js`; package tests; `scripts/check-dependency-boundary.mjs` | Default | uncovered | merge-blocking; claim-blocking | Governing consumer/need review for future API additions remains manual. |
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Phase 7A-7C are committed focused increments; Phase 7D adds formal run sealing and bounded scoring/standing without performing a real campaign or changing SUT behavior. | Rule entry | Section diff; repeated hostile passes; targeted authority/evidence/run/scoring suites; full local gates; historical reviews remain scoped to named ranges. | Phase 7A-7D source/tests and records; prior review closures | Implementing-task red-team and local gates; fresh independent exact-commit reviews pending. | uncovered | advisory or merge-blocking by condition | The implementing task cannot independently attest its Phase 7 commits; protected required-check configuration remains unverified. |
...
| `ENG-HEALTH-FAILURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Malformed or ambiguous state fails closed; Phase 7 also rejects malformed custody, authority replay, run outcomes/findings, incomplete history, favorable aggregation, standing forks/reopens, and circular or non-head result authority. | Rule entry | Existing typed/rollback attacks plus exact-byte, authority, evidence/run, historical closure, aggregation recomputation, early-stop, standing, supersession, and namespace attacks. | SUT/evaluation source and package tests; Phase 7 modules/tests; repository gates | Default plus formal-evaluation local-recorded/CI-required; protected-check verification pending. | uncovered | merge-blocking; claim-blocking | Environmental interruption may leave harmless unreferenced content-addressed bytes; no incomplete artifact receives a reference or authority. |
| `ENG-HEALTH-GEN-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The projector generated standard `V0.6.4`, profile `V0.5.0`, integration `V0.2.2`, three new ADR snapshots, `OPEN_QUESTIONS.md V0.2.23`, lock metadata, source closure, and strengthened checker content from canonical commit `a77a431`. | Rule entry | Transactional projector validation; digest verification; exact ledger mapping audit; local tests/gates; qualifying independent review of exact workbench commit `2385020`. Contract mode. | Projected `governance/` tree; `npm run check`; `governance/ZOEY_GOVERNANCE.lock`; independent-review and owner-approval closure below | Qualifying independent generated-artifact review acknowledged by the project owner and recorded below. | review-only | merge-blocking; promotion-blocking | The reviewed projection is closed for this baseline; future generated governance or implementation artifacts require fresh explicit review. |
...
| `ENG-HEALTH-STRUCTURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | SUT closure/checkpoints remain unchanged; configuration, prospective authority, durable evidence/run records, and scoring/standing remain separate evaluator modules with explicit exact-reference transitions. | Rule entry | Static dependency direction; closed construction by artifact family; durable replay at authority/scoring transitions; targeted boundary, capture, run, and aggregation attacks. | Existing bounded modules; Phase 7A-7D modules/tests; dependency gate | Implementing-task structural review; fresh independent Phase 7 structure review pending. | uncovered | advisory or merge-blocking by condition | The full Phase 7 evaluator boundary requires independent review; it remains workbench-specific and non-permanent. |
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Regressions cover the complete development trajectory and Phase 7A-7D configuration, authority, custody/replay, formal runs, indexes, aggregation, result authority, and standing mechanisms. | Rule entry | Existing behavior suites plus Phase 7 contract, negative, property, replay, fork, completeness, privacy, recomputation, historical, and standing tests; full aggregate gate. | SUT/evaluation tests; all Phase 7 test suites; conformance tests; `package.json` | Default plus formal-evaluation local-recorded/CI-required; protected-check verification pending. | uncovered | merge-blocking; promotion-blocking | Phase 7 mechanisms are locally tested but await fresh independent exact-commit review; all test artifacts remain synthetic and non-formal. |
| `ENG-HEALTH-TEST-002` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Canonical development trajectories use the development harness and declared public SUT boundary; private-helper tests are limited to malformed retained-state, creator/cardinality, transaction, and no-mutation attacks unreachable through valid public input. | Rule entry | End-to-end delivery, output/simulator routing, passive checkpoints, outcome/explanation closure, and hostile parity tests; raw fixtures contain no SUT conclusions; internal attacks do not become behavior or formal evidence. | `scn001_sut_core/test/`; `scn001_eval/test/`; `tests/conformance/`; independent closures below | Default | uncovered | merge-blocking; claim-blocking | Bounded test validity received independent review; tests remain engineering/conformance evidence only, and protected required-check configuration remains unverified. |
...
Historical-record boundary: every change-specific and review section below,
through `Fixture-Initialization Hardening Independent Review Closure`, records
the state at its named commit or range. Its old commit identities, rule counts,
version transitions, implementation scope, and then-current terminology do not
describe current standing. The later `ADR-010 Through ADR-012 Projection
Pre-Phase-7 Revalidation` section summarizes the current migration context, but
the table above remains the sole authoritative disposition.
...
retained state. The development harness proves canonical creation and exact replay
...
* canonical development-harness creation and replay;
...
The canonical development path continues to create:
...
The development harness passively requires one exact completed focused-drill prefix
...
Exact development replay reuses one staged bundle, correction state, disposition,
...
* the development acceptance branch remains restricted to one exact selected candidate/proposal checkpoint;
...
2. the development acceptance package and SUT classifier did not require raw-response
...
The generic development fixture path now rejects every `user_response` before fixture
...
* development evaluation-harness constructor shape;
...
The review confirms that the development package-root evaluation API exports only `createEvaluationHarness`. The development harness constructor accepts only the SUT public boundary and rejects additional renderer/projector override arguments. Development harness construction therefore uses the governed deterministic proposal renderer and simulator projector rather than an arbitrary caller-supplied replacement. Renderer/projector failure injection remains confined to the package-internal mechanism-test construction path and is not exposed from `scn001_eval/index.js`.
...
The development harness passively requires one exact active production-focused trial,
...
independent-run isolation; and end-run clearing. The development constructor remains
...
source-binding ledger enforces the same rule before committing an evaluation-source
binding candidate.
...
This historical migration added lock coverage for the four new Phase 7 rules.
Their sole authoritative rows now appear in the complete table above. The
current implementation has not crossed their applicability triggers, so each
remains `not-applicable`/`N/A` and must become `applicable` with mapped
mechanisms before the first named artifact or operation is implemented.

## Fixture-Initialization Hardening Independent Review Closure

On 2026-07-20, a fresh independent review examined implementation/test range
`6e57840126b490805032e350e2735e55dabc6b8b..6d8b41d28471b0ec2ba9cbba4793e7cb0d9eb4fa`
for the post-closure fixture-initialization hardening. Historical governance
projection text within that range is superseded by the current lock and is not
attested by this implementation review.

Review outcome for the initialized-batch implementation: **pass with no
blocking finding**.

The reviewer independently confirmed that:

- SUT retained-input closure and evaluator source-binding closure compute the
  same ordered set of newly retained fixture-initialized communications;
- each such communication requires exactly one valid initialized assertion and
  every additional ingestion result must be one of those assertions;
- duplicate result references, omitted assertions, assertion duplication for
  one source, and order swapping fail closed in both domains;
- a redelivered already-retained communication is excluded from the new-
  assertion set while a newly retained communication in the same interaction
  still requires exactly one assertion;
- evaluator validation commits no partial source binding on failure and a clean
  retry remains possible;
- run-local identity, source actor, interaction, transition, typed source/basis
  relations, first-ingestion identity, and chronology remain part of each
  assertion's validation; and
- the change neither expands the public SUT API nor creates formal evaluation,
  compatibility, scoreability, result, completion, or production claims.

Current verification reran `npm run check`: governance, conformance, formatting,
syntax, dependency, boundary, state, and aggregate gates passed with 407 tests,
none failed or skipped. Both the reviewed-range and current-worktree
`git diff --check` passed. Hosted required-check/branch-protection status was not
observed and is not claimed.

This closes the previously pending change-specific implementation review. It
makes the current code eligible to serve as a prospective Phase 7 development
baseline after the separate governance projection reviews close. It does not
promote any existing run, trace, checkpoint, capture, or artifact into formal
evidence; those artifacts remain permanently non-formal under `ADR-011 R3`.

## ADR-010 Through ADR-012 Projection Pre-Phase-7 Revalidation

The workbench now consumes the clean canonical baseline at meta commit
`a77a431`:

- `ENGINEERING_STANDARD.md V0.6.4`;
- selected-slice profile `V0.5.0`;
- Codex integration `V0.2.2`;
- `OPEN_QUESTIONS.md V0.2.23`; and
- exact accepted snapshots `ADR-010 R3`, `ADR-011 R3`, and `ADR-012 R3`.

The lock contains 53 unique rules. The strengthened checker verifies exact
ledger-to-lock rule revision/source mapping, rejects duplicate rows, validates
applicability/status values, and requires consistent `not-applicable`/`N/A`
dispositions. Projection, source digests, agent routing, and complete index
coverage pass.

The current repository has not crossed the triggers for
`ENG-CONF-CONFIG-001 R1`, `ENG-CONF-AUTHORITY-001 R1`,
`ENG-CONF-EVIDENCE-002 R1`, or `ENG-CONF-SCORE-001 R1`; all four are explicitly
`not-applicable` with exact future triggers. They must be remapped to
`applicable` before Phase 7 implements their first manifest, authorization,
formal evidence/index, or bounded aggregation artifact.

Current applicability/status counts are:

- 44 rules `applicable` and 9 `not-applicable`;
- 4 applicable rules `review-only`;
- 33 applicable rules `uncovered`;
- 7 applicable rules `revalidation-required`;
- no rule claimed `enforced`.

The seven source-bound rows awaiting qualifying human review are
`ENG-BASE-002 R2`, `ENG-BASE-REPO-001 R2`, `ENG-CHANGE-001 R5`,
`ENG-CLAIM-001 R5`, `ENG-CLAIM-002 R5`, `ENG-CLAIM-WORKBENCH-001 R2`, and
`ENG-CONF-CLAIM-001 R5`. `ENG-AGENT-CODEX-003 R2` remains `uncovered` pending
the required human instruction-refresh review. `ENG-HEALTH-GEN-001 R2` remains
`uncovered` pending review independent of the task that generated the
projection.

These pending reviews block Phase 7 start under the accepted ADR acceptance
effects. They do not reopen the ADRs, invalidate the clean projection, or make
the four future Phase 7 controls currently applicable. Existing protected
required-check configuration also remains unverified; no promotion or formal
claim may rely on it until verified.

## Phase 7 Source-Bound Owner Approval Closure

Review and approval date: 2026-07-21.

Exact reviewed workbench commit:
`2385020961e66c64edbf332ec07b3f7ff809b089`.

Canonical meta commit:
`a77a431ad1595a45db80c2f67a634971bebc7739`.

Qualifying independent-review disposition: **PASS**.

Project-owner disposition: **ACCEPTED**.

The project owner reviewed the qualifying independent assessment of the exact
workbench commit and did not rely on the correction task's non-qualifying
self-review as the required independent review. The owner approved the
source-bound mappings against `ADR-009 R4`, `ADR-010 R3`, `ADR-011 R3`,
`ADR-012 R3`, `OPEN_QUESTIONS.md V0.2.23`, engineering standard `V0.6.4`,
selected-slice profile `V0.5.0`, and Codex integration `V0.2.2`.

The approval confirms that the mappings do not weaken behavior-configuration
or evaluation-configuration identity, prospective campaign authority, evidence
or namespace completeness, qualification or run-count authority, bounded-
result or result-standing semantics, scoreability or claim ceilings, the
`P -> D -> A` completion dependency, the prohibition on retroactive promotion,
or the scenario-provisional workbench boundary.

The owner approved these exact source-bound status transitions:

- `ENG-BASE-002 R2`: `revalidation-required` -> `uncovered`;
- `ENG-BASE-REPO-001 R2`: `revalidation-required` -> `review-only`;
- `ENG-CHANGE-001 R5`: `revalidation-required` -> `review-only`;
- `ENG-CLAIM-001 R5`: `revalidation-required` -> `uncovered`;
- `ENG-CLAIM-002 R5`: `revalidation-required` -> `uncovered`;
- `ENG-CLAIM-WORKBENCH-001 R2`: `revalidation-required` -> `review-only`; and
- `ENG-CONF-CLAIM-001 R5`: `revalidation-required` -> `uncovered`.

The owner also approved the refreshed instruction chain required by
`ENG-AGENT-CODEX-003 R2`, closing its human-review condition and changing its
status from `uncovered` to `review-only` for this baseline.

The same independent review verified the generated projection, exact canonical
artifacts, lock digests, and rule mapping at the reviewed commit. The owner
acknowledged that review as the qualifying generated-artifact review for
`ENG-HEALTH-GEN-001 R2`, changing its status from `uncovered` to `review-only`.

Current authoritative status counts after this closure are:
...
The four new Phase 7 implementation rule families remain
`not-applicable`/`N/A` until their actual prospective triggers occur:

- `ENG-CONF-CONFIG-001 R1`;
- `ENG-CONF-AUTHORITY-001 R1`;
- `ENG-CONF-EVIDENCE-002 R1`; and
- `ENG-CONF-SCORE-001 R1`.

Each triggered row must become `applicable` and map its actual mechanisms,
tests, promotion integration, evidence, reviewer, and residual risk before
implementation relies on it.

This closure does not report any uncovered rule as enforced, establish missing
automated or protected-branch controls, or satisfy unverified `CI-required`
integration. It does not promote development tests, runs, traces, captures,
checkpoints, reports, or harness activity into formal evidence.

It does not authorize a formal campaign, designate an effective campaign
namespace, authorize actual formal execution, establish a bounded campaign
result or result standing, create completion evidence package `P`, determine
completion eligibility `D`, record owner milestone disposition `A`, declare
milestone completion, establish formal campaign readiness, or establish broader
`SCN-001` acceptance or production readiness.

No behavior/evaluation manifest, qualification plan or result, campaign
authorization, formal attempt or evidence, namespace or campaign index,
bounded-result aggregation, result-standing artifact, or scoreability code is
created by this governance-review closure.

With the review records and truthful statuses recorded, Phase 7 may become
`READY_FOR_PHASE7_IMPLEMENTATION` after this focused closure passes the complete
local gate. That status means readiness to begin governed implementation only;
it is not formal campaign readiness or any stronger authority, result,
completion, acceptance, or production claim.

## Phase 7A Configuration Identity Implementation

Implementation date: 2026-07-21.

Starting workbench commit: `e6e1dc1`.

Phase 7A activates `ENG-CONF-CONFIG-001 R1` and implements only the
configuration-identity layer required by `ADR-010 R3` and `ADR-011 R3`:

- strict JSON-domain canonicalization compatible with the accepted
  `RFC8785-JCS` contract and SHA-256 domain separation;
- the closed V1 artifact-kind/schema, decision-kind, evidence-kind, and hash-
  domain catalogues;
- immutable behavior- and evaluation-configuration manifest builders with
  separately recomputed payload and manifest-assertion fingerprints;
- closed SUT/evaluator source, build, dependency, runtime, environment,
  boundary, policy, fixture/oracle, run-count, governing-basis, custody, and
  typed-applicability identity shapes;
- exact manifest references and exact artifact resolution;
- immutable correction/supersession lineage; and
- fail-closed behavior/evaluation comparison outcomes.

The implementing task performed three explicit hostile passes. The resulting
fixes reject accessor-backed or decorated JSON, sparse/non-finite/invalid-
Unicode inputs, incomplete fixture paths or claim classes, missing governing
basis, mutable model/tool/provider aliases, prompt content without custody,
ID-only or kind/schema-mismatched references, wrong-domain fingerprints,
unresolved exact references, unordered identity sets, and tampering with
payload, provenance, creator/time, hash metadata, or correction lineage. SUT
source closure cannot absorb evaluator/governance-only files or depend on the
evaluator package, and package projections are scoped explicitly. Prompt
composition remains an ordered sequence rather than being incorrectly
normalized as a set.

The complete local gate passed with 417 tests, none failed or skipped. The
targeted Phase 7A suite contributes 10 contract/property/negative/regression
tests. Governance, formatting, syntax, dependency, boundary, state, and
aggregate gates also passed. Protected required-check configuration was not
verified and is not claimed.

This change creates code and synthetic test objects only. It does not publish a
behavior/evaluation manifest, attest that caller-supplied source digests match
repository bytes, choose an effective namespace, anchor a qualification plan,
authorize a campaign, create formal evidence, aggregate a result, or establish
any formal claim. Existing development runs and artifacts remain permanently
non-formal.

Current authoritative status counts for this unreviewed Phase 7A change are:

- 45 rules `applicable` and 8 `not-applicable`;
- 8 applicable rules `review-only`;
- 37 applicable rules `uncovered`;
- no applicable rule `revalidation-required`; and
- no rule claimed `enforced`.

Fresh independent review of the exact Phase 7A commit is pending. That review
condition blocks formal reliance on this layer, but it does not activate
`ENG-CONF-AUTHORITY-001`, `ENG-CONF-EVIDENCE-002`, or
`ENG-CONF-SCORE-001`; those rules remain `not-applicable` until their distinct
implementation triggers occur.

## Phase 7B Prospective Authority Implementation

Implementation date: 2026-07-21.

Starting workbench commit: `e947dfe`.

Phase 7B activates `ENG-CONF-AUTHORITY-001 R1` and implements the prospective
authority layer required by `ADR-011 R3` and `ADR-012 R3`:

- closed generic artifact envelopes for only the implemented qualification,
  authorization, and evaluation-decision families;
- prospectively allocated deterministic-qualification plans with at least two
  independent executions per required path and no retry-into-qualification;
- sealed qualification results retaining every execution/comparison, exact
  capture/start/validation references, distinct validation, and mechanically
  derived `QUALIFIED`, `NOT_QUALIFIED`, or `INCONCLUSIVE` status;
- campaign authorizations that repeat the evaluation manifest's sole-authority
  path/claim/policy fields and derive one- or three-run branches exactly;
- prospective primary and divergence-contingency attempt allocation, known-
  exploratory-output exclusion, and the accepted invalid replacement limits;
- exact anchor/index/receipt and causal fresh-start prerequisite validation;
- immutable initial run-invalidity decisions plus deterministic replacement
  allocation; and
- immutable material authority changes requiring distinct review and explicit
  project-owner acceptance.

Three hostile passes corrected material defects: an early version returned a
`started` grant before durable index/receipt resolution, allowed reuse of
qualification captures, failed to close proof cardinality and cross-slot
identity, did not bind invalidity to its allocation, allowed generic
construction of under-specified evidence/run/index artifacts, and did not
require a correction decision for qualification-result supersession. The final
preflight returns `execution_start_authorized: false` and only
`READY_FOR_DURABLE_AUTHORITY_RESOLUTION`; 7C must resolve the exact namespace
index, raw anchor receipt, and durable fresh-start capture before execution can
start.

Additional attacks reject incomplete qualification paths, favorable retries or
result labels, actor collapse, mutable policy retargeting, omitted executions,
run-count selection, narrowed campaigns, hard-failure replacement, reused or
unallocated attempts, receipt production by the result producer, replayed
start events, replacement-budget/root-cause bypass, mismatched decision
subjects, self-review, and non-owner material decisions.

The complete local gate passed with 428 tests, none failed or skipped. The
targeted Phase 7B suite contributes 11 contract/negative/replay/regression
tests. Governance, formatting, syntax, dependency, boundary, state, and
aggregate gates also passed. Protected required-check configuration was not
verified and is not claimed.

This change implements authority schemas and structural mechanisms only. Test
objects are synthetic and non-formal. No namespace or receipt artifact is
created, no external anchor is performed, no SUT execution is started, no
formal evidence is captured, and no campaign result or claim exists. Actual
formal execution still requires campaign-specific owner inputs and external
events after 7C closes durable authority resolution.

Current authoritative status counts for this unreviewed Phase 7B change are:

- 46 rules `applicable` and 7 `not-applicable`;
- 6 applicable rules `review-only`;
- 40 applicable rules `uncovered`;
- no applicable rule `revalidation-required`; and
- no rule claimed `enforced`.

Fresh independent review of exact commits `e947dfe` and the resulting Phase 7B
commit remains pending before formal reliance. `ENG-CONF-EVIDENCE-002` and
`ENG-CONF-SCORE-001` remain `not-applicable`; the generic constructor rejects
their artifact families until their closed schemas are implemented in 7C/7D.

## Phase 7C Durable Evidence And Index Closure

Implementation date: 2026-07-21.

Starting workbench commit: `6d2beed`.

Phase 7C activates `ENG-CONF-EVIDENCE-002 R1` and implements the durable
evidence/authority-resolution layer required by `ADR-011 R3`:

- a bounded file-backed store for exact raw bytes and canonical typed artifacts,
  with content-addressed paths, immutable writes, byte length/digest replay,
  symlink/path escape rejection, and canonical-storage verification;
- closed evidence semantics for SUT input/output/inspection, simulator request/
  realization, evaluator-private captures, exact anchor receipts, and raw
  binary attachments without changing SUT visibility or behavior;
- distinct evidence producer/validator sealing and exact campaign, attempt,
  path, order, media, semantic, and custody bindings;
- cumulative independently validated authority-namespace revisions with exact
  manifest bindings, prospective anchor declarations, consecutive predecessor
  closure, single-head resolution, and fork quarantine;
- frozen independently validated campaign indexes that retain every authorized,
  unstarted, invalid, replacement, sealed, and otherwise dispositioned attempt
  and reconcile all local run/evidence/decision members with their global
  lists; and
- the only implemented transition from the Phase 7B ungranted preflight to
  `execution_start_authorized: true`, after complete typed authorization,
  configuration/qualification, namespace-head, external receipt, raw custody,
  slot, event, and fresh-start reconstruction.

Three hostile passes corrected early gaps that would have allowed loosely typed
manifest bindings, generic rather than full authorization validation,
campaign-member omissions, non-consecutive namespace ancestry, caller-supplied
namespace references without resolution, arbitrary evidence semantics,
noncanonical stored artifacts, and insufficient producer/validator closure.
Additional attacks reject reconstructed strings in place of bytes, path escape,
evaluator-private SUT-visible fields, contradictory simulator fidelity,
unstored artifacts, forged preflights, stale namespace heads, competing forks,
dropped cumulative history, omitted authorized slots, and local/global member
mismatch.

The complete local gate passed with 435 tests, none failed or skipped. The
targeted Phase 7C suite contributes 7 contract/negative/replay/regression tests;
the combined Phase 7B/7C targeted run passed 18 tests. Governance, formatting,
syntax, dependency, boundary, state, and aggregate gates also passed. Protected
required-check configuration was not verified and is not claimed.

All evidence, namespace, authorization, and index objects created by tests are
synthetic mechanism tests and permanently non-formal. This change does not
designate a real effective namespace, publish an external anchor, start a real
SUT campaign, create a formal run record, aggregate or validate a bounded
result, establish result standing, or support any behavioral/milestone claim.

Current authoritative status counts for this unreviewed Phase 7C change are:

- 47 rules `applicable` and 6 `not-applicable`;
- 6 applicable rules `review-only`;
- 41 applicable rules `uncovered`;
- no applicable rule `revalidation-required`; and
- no rule claimed `enforced`.

Fresh independent review of exact Phase 7A-7C commits remains pending before
formal reliance. `ENG-CONF-SCORE-001` remains `not-applicable`; Phase 7C does
not construct formal run records or classify any run/campaign outcome, so the
scoreability trigger remains reserved for Phase 7D.

## Phase 7D Formal Run And Bounded Scoreability Closure

Implementation date: 2026-07-21.

Starting workbench commit: `ba66fcc`.

Phase 7D activates `ENG-CONF-SCORE-001 R1` and completes the accepted
implementation-only Phase 7 artifact chain under `ADR-011 R3` and `ADR-012 R3`:

- sealed formal run records bind exact durable start authority, attempt/seed
  allocation, initial-state and isolation proofs, ordered SUT/simulator
  evidence, provider handles, and distinct producer/validator sealing;
- validity, invariant result, five-class obligation vector, lifecycle, and
  failure findings remain orthogonal, while invalid runs fabricate no scored
  SUT outcome and every valid finding binds a closed rule and exact run evidence;
- durable replay reconstructs the original authorization through its effective
  namespace history rather than incorrectly requiring that historical start
  revision to remain the current head;
- frozen campaign scoring mechanically applies the exact one/three-run branch,
  all four paths, five claim classes, closed pressure map, and nonnumeric
  conjunctive aggregation, with no voting, weighting, or class substitution;
- missing authority/evidence/coverage remains `NOT_YET_DETERMINABLE`, while an
  exact valid hard failure or complete mandatory path/class nonpass yields
  `BOUNDED_FAIL`; the only early closure requires exact never-started
  dispositions tied to the decisive run and finding;
- every campaign index in the closure namespace requires attribution context;
  authoritative historical hard failures survive evaluation-only change, and
  an unchanged behavior/evaluation failure cannot be erased by repetition;
- bounded claims carry the exact synthetic ceiling and `GROW-002` exclusion;
  claim authority remains pending until a later namespace indexes the immutable
  result and complete owner-reviewed standing chain under an external receipt;
  and
- result standing is monotonic and fork-intolerant, `SUPERSEDED` is terminal,
  supersession requires its complete durable predecessor/evidence chain, and no
  result/index dependency cycle is accepted.

Repeated adversarial passes corrected loose historical attribution,
non-pressure history misclassification, stale-head replay, replacement seed/
selection conflation, unused-slot cross-campaign references, incomplete
standing chains, exact-reference aliasing, terminal-standing reopening, and
caller-trusted run replay. Tests also reject incomplete coverage becoming SUT
failure, favorable recomputation, weighted scoring, vote-based recovery,
incorrect early cancellation, missing later indexing, standing forks, actor
collapse, cross-attempt evidence, and fabricated invalid-run outcomes.

The complete local gate passed with 451 tests, none failed or skipped. The new
Phase 7D formal-run and bounded-scoring suites contribute 16 contract,
negative, replay, aggregation, authority, and regression tests. Governance,
formatting, syntax, dependency, boundary, state, and aggregate gates passed.
Protected required-check configuration was not verified and is not claimed.

Every manifest, authorization, namespace, capture, run, campaign index, result,
receipt, and standing decision created by these tests is synthetic mechanism
data and permanently non-formal. This implementation does not designate a real
namespace, perform an external anchor event or fresh formal execution, publish
a real bounded result, establish authoritative standing, authorize a selected-
slice claim, complete the milestone, or justify durable-project extraction.

Current authoritative status counts for this unreviewed Phase 7D change are:
...
Fresh independent review of exact Phase 7A-7D commits remains pending before
formal reliance. No owner campaign input is needed to finish implementation,
...
## Phase 7 Configuration-Measurement Corrective Addendum

Corrective date: 2026-07-22.

Starting implementation head: `5f22713`.

Current corrective state:
`CONFIGURATION_MEASUREMENT_CORRECTED_REVIEW_PENDING`.

The corrective change verifies a clean exact-`HEAD` worktree before asserting
clean provenance; replaces the private `../../meta` test assumption with
isolated temporary Git repositories; recursively inventories runtime source;
rejects unknown, symlinked, or otherwise non-regular runtime members; checks
relative static-import closure and declared dependency closure; and proves the
four-name behavior-affecting environment catalogue against evaluator source
access. The trusted authorization component reconstructs the supplied behavior
and evaluation manifests from the exact commit, compares the complete manifest
artifacts, rejects qualification-supported input, and derives the conservative
three-run authorization branch. Exact-reference lists additionally reject a
repeated `(artifact_kind, artifact_id)` identity even when fingerprints differ.

Hostile tests cover dirty/historical/unavailable revisions, standalone layout,
nested source, unclassified members, import and environment escapes, source
symlinks, manually reconstructed valid-but-invented manifests, and conflicting
graph identities. These mechanisms remain non-public pipeline components and
synthetic test infrastructure. Later corrective increments now implement the
internal oracle and evidence-trust primitives, while factual invalidity replay,
runner-to-record integration, qualification execution derivation, interruption
retention, and the bounded formal campaign runner remain open.

This addendum records implementing-task evidence only. It does not reverse the
independent `BLOCKED` disposition, change any of the four Phase 7 controls from
`applicable / uncovered`, establish a hosted/protected check, or authorize a
campaign. The complete corrective range requires a green aggregate gate and
fresh exact-commit independent review.

## Phase 7 Formal-Execution Oracle-Core Corrective Addendum

Corrective date: 2026-07-22.

The corrective execution core creates the selected-slice SUT boundary itself,
executes every accepted path through that public boundary, and records each
delivery, processing step, simulated realization, exact source fixture
material, and post-event SUT inspection under one recorder-owned monotonic
sequence. It includes initialized history, current resume attribution,
canonical calibration and trajectory, equal-calibration non-activation, the
same-run drill-opt-in prefix, and recent-basis temporal pressure. The oracle
derives the exact five-class vector and downstream `NOT_REACHED` state; its
closed input does not accept run validity, invariant, obligation, or finding
fields. A missing-material-output attack produces an obligation failure and
cannot be selected into a pass.

The change also corrects exact original-ingestion replay for declared batches
that create fixture-initialized assertions: the ingestion may name the exact
complete initialized-assertion result set, while missing, extra, reordered, or
misbound initialized assertions continue to fail closed. Existing hostile
checkpoint suites and the new actual-path tests cover that integration.

This is an evaluator-internal execution/oracle core, not a package-root formal
API. The later evidence-trust increment supplies typed durable proof and anchor
primitives, but this transcript is not yet connected to them as a sealed run;
interrupted-attempt retention and exact runner-to-record integration remain
open. The four Phase 7 controls remain `applicable / uncovered`, the prior
`BLOCKED` review stands, and no campaign or formal claim is authorized.

## Phase 7 Evidence-Trust Corrective Addendum

Corrective date: 2026-07-22.

The prospective anchor policy now closes an Ed25519 verifier key ID and public-
key fingerprint. Receipt capture parses exact canonical signed bytes and derives
the stored semantic envelope; durable authority replay authenticates the
receipt and each applicable namespace predecessor instead of accepting
caller-authored external labels. Synthetic hostile tests reject a substituted
key, signed-payload mutation, noncanonical bytes, and receipts without the
owner-pinned verifier.

The internal formal-evidence recorder owns a single monotonic attempt event
sequence and derives capture role, digest, delivery position, and event identity.
Closed canonical proof documents cover fresh start, initial state, run
isolation, prospective selection, qualification fresh start, qualification
oracle capture, and validation attestation. Strict raw replay reconstructs each
proof envelope, and formal-run validation checks the shared run scope and exact
slot selection/seed basis. Qualification validation additionally binds the
exact plan/execution/comparison/result/anchor/fresh-start basis.

This remains synthetic mechanism evidence. No project verifier key, effective
namespace, real external event, formal attempt, or bounded result has been
designated or created. Qualification comparisons are not yet derived by a
trusted execution runner. The formal-attempt increment below closes the four-
path runner-to-record transition, while qualification and full campaign closure
remain open. The four Phase 7 controls remain `applicable / uncovered`; the
independent `BLOCKED` disposition stands until the remaining pipeline closes
and receives a fresh exact-commit review.

## Phase 7 Formal-Attempt Pipeline Corrective Addendum

Corrective date: 2026-07-22.

The internal attempt pipeline now creates the selected-slice session and exact
run scope before obtaining fresh-start evidence. An external adapter must return
canonical Ed25519-signed bytes bound to the exact authorization, effective
namespace, anchor receipt, prospective slot/attempt/path, anchor event, and
recorder-owned run scope. The pinned-key verifier and exact-context comparison
complete before the first path event; unsigned bytes and a correctly signed but
substituted run scope fail closed.

After durable authority resolves, the pipeline converts the runner transcript
into typed byte-exact SUT input, SUT output, simulator request/realization, and
post-event inspection custody under recorder-owned chronology. It derives
formal findings from runner assertions and exact inspection references, then
seals only runner-owned validity, invariant, and five-class obligation results.
An injected execution exception retains all evidence captured through the point
of interruption, records the exact closed invalidity reason, marks the attempt
`ABANDONED_RETAINED`, and creates no scored run. A missing material output seals
as an obligation failure with downstream `NOT_REACHED`; formal-run validation
requires exact findings for failures while allowing finding-free `NOT_REACHED`.

Targeted evidence, run-record, and attempt-pipeline tests pass, and the complete
local gate passes 471 tests with governance, formatting, syntax, dependency, and
conformance checks. These remain synthetic mechanism tests and no artifact is
retroactively promotable. Qualification execution/comparison derivation,
complete bounded multi-slot orchestration, result/decision publication, the
project-owner key/effective namespace and external events, protected integration,
and fresh independent exact-commit review remain open. The prior `BLOCKED`
disposition and all four `applicable / uncovered` control statuses remain
unchanged.
...
The internal trusted qualification path now resolves the exact stored plan and
effective namespace, authenticates its externally signed anchor, and verifies
the closed comparator identity before creating any execution. It runs both
prospectively allocated sessions for all four paths. Each session requires an
Ed25519-signed start bound to the exact plan, namespace, receipt, execution,
path, selection basis, anchor event, and actual run scope.

Each oracle proof retains canonical exact execution material and its complete
normalized comparator input. Typed replay recomputes both bound digests. The
closed comparator alpha-renames only run-local UUID identities in deterministic
first-occurrence order, then applies exact JCS equality. Pair comparisons and
the final qualification classification are derived by the runner: actual repeat
executions close as `QUALIFIED`, while a real missing-output divergence yields
an exact `VALUE_MISMATCH` and `NOT_QUALIFIED`. No input accepts a qualification
classification or comparison.

The complete local gate passes 474 tests. All qualification executions, keys,
receipts, and artifacts remain synthetic and non-promotable. Complete bounded
campaign orchestration and publication, project-owner effective authority,
protected integration, and fresh exact-commit independent review remain open;
the prior `BLOCKED` disposition and all four `applicable / uncovered` controls
remain unchanged.
...
The internal trusted campaign runner now closes the implementation chain from
prospective campaign authorization through actual attempt execution, exact
evidence/run custody, frozen attempt inventory, durable campaign index, successor
authority namespace, externally authenticated namespace receipt, and independently
derived bounded result. It executes exactly four primary slots for the qualified
branch and twelve for the conservative branch. No input accepts attempt outcomes,
aggregate results, or standing.

Regression tests execute both complete branches and derive `BOUNDED_PASS`. The
runner returns only `RESULT_PENDING_OWNER_STANDING`, with no standing artifact and
`claim_authorized: false`; `CURRENT` standing remains a later independent-review
and project-owner act followed by result indexing. The complete local gate passes
477 tests with governance, formatting, syntax, dependency, and conformance gates.

This historical implementing-task status is superseded by the integration
red-team correction below. All objects remain synthetic and non-promotable.

## Phase 7 Integration Red-Team Correction Addendum

Corrective date: 2026-07-25.

The corrected trusted entry binds measurement to the repository that loaded the
evaluator and permits only the measured conservative V1 branch. Durable
qualification verification reconstructs comparator inputs from replayed oracle
material, re-derives all pair classifications and the final result, and rejects
supplied favorable labels. Formal attempts retain one pre-proof raw initial
inspection; run replay resolves both initial/isolation proofs to those canonical
bytes and reconstructs emptiness, run scope, and exact digests.

The bounded runner no longer hardcodes campaign history. Its explicit history
context must close exactly every prior campaign index in the authorizing
namespace before an attempt executes. A qualified mechanism run is compared
with its retained qualification baseline; divergence halts before campaign
index or bounded-result creation and returns the exact accepted-decision basis.
A fixture-delivery interruption records exact attempted source material and the
closed `VAL-002` predicate without claiming that the bundle was delivered. The
raw observation is replayed before an immutable `INVALID_UNSCORABLE` run can
receive its validator-confirmed invalidity decision. The signed replacement
start binds the complete derived allocation, including its exact decision and
predecessor references. The runner executes that replacement and suspends at
the accepted repeated-root, per-path, and campaign-wide limits.

Failure attribution is closed separately. A SUT processing failure after
material delivery is retained as a valid scored obligation failure and cannot
buy a retry. A fixture withheld because its SUT prerequisite is absent is
recorded privately and is not mislabeled as delivered. A post-start capture or
seal failure that cannot satisfy formal-run integrity remains
`ABANDONED_RETAINED` and produces `authority_review_required`; it is never
upgraded merely to unlock replacement authority.

The complete local gate passes 489 tests with none failed, skipped, or
cancelled; governance, formatting, syntax, dependency, boundary, and state
checks pass. This is implementing-task evidence, not independent review or
formal campaign evidence.

The authoritative status is
`PHASE7_MECHANISM_LAYER_COMPLETE_PIPELINE_CLOSURE_PENDING`; subordinate status
is `PHASE7_MEASURED_CONSERVATIVE_PIPELINE_IMPLEMENTED`. The owner-governed
resume/supersession transition after qualified-branch divergence remains open.
The four Phase 7 controls remain
`applicable / uncovered`. No real campaign or claim is authorized, and fresh
exact-commit independent review, protected integration, project-owner authority,
and real external events remain mandatory.
