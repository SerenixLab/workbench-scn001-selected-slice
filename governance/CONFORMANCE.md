...
| `ENG-CONF-AUTHORITY-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7B implements immutable qualification plans/results, mechanically derived campaign authorization, outcome-independent attempt/replacement allocation with separate seed commitment, structural anchor/fresh-start prerequisites, invalidity, and governed decisions. Phase 7C/7D grant or replay a start only after exact durable receipt, raw fresh-start, configuration, qualification, and effective namespace-history resolution. | `ADR-004 R3`; `ADR-005 R2`; `ADR-011 R3`; `ADR-012 R3` | Contract, negative, replay, and regression tests over authority derivation, replacement seed closure, full typed authorization reconstruction, exact indexed bindings, raw receipt/start custody, namespace history, and preflight-forgery attacks. | `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/formalAuthority.js`; `scn001_eval/src/formalEvidence.js`; `scn001_eval/src/formalRunRecord.js`; corresponding tests; Phase 7B-7D records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Synthetic tests exercise start and replay transitions, but no real effective namespace, external anchor event, campaign execution, or independent exact-commit review exists. |
...
| `ENG-CONF-EVIDENCE-002` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7C/7D implement immutable closed evidence envelopes; exact raw-byte custody/replay; cumulative namespace/frozen campaign indexes; complete attempt inventories; durable start authority; and sealed formal run records with exact evidence, control, simulator, allocation, outcome, and failure-finding closure. | `ADR-002 R2`; `ADR-008 R2`; `ADR-011 R3` | Contract, negative, replay, and regression tests cover bytes, envelopes, privacy, simulator fidelity, authority reconstruction, namespace history/forks, attempt closure, run evidence partition/order, run isolation, orthogonal validity/invariant/obligation outcomes, and exact findings. | `scn001_eval/src/formalEvidence.js`; `scn001_eval/src/formalRunRecord.js`; corresponding tests/support; Phase 7C/7D records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Tests use synthetic objects only; no actual external anchor/campaign/run exists, no test artifact is promotable, and exact-commit independent review is pending. |
...
synthetic test infrastructure. Formal outcome derivation, factual invalidity
replay, proposition-specific proof generation, recorder-owned chronology,
authenticated external authority, raw semantic-envelope replay, and the bounded
formal runner remain open.
...
API. Its transcript is not yet typed durable evidence or a sealed run; external
start authentication, proposition-specific control proofs, interrupted-attempt
retention, raw semantic replay, and exact formal-record integration remain
