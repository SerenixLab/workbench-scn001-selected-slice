...
| `ENG-CLAIM-002` | `R5` | `ENGINEERING_STANDARD.md` | applicable | Repository artifacts discuss Phase 7 but explicitly deny formal status and retroactive promotion; no accepted V1 formal artifact exists yet. | `OPEN_QUESTIONS.md V0.2.23`; `ADR-004 R3`; `ADR-005 R2`; `ADR-009 R4`; `ADR-010 R3`; `ADR-011 R3`; `ADR-012 R3` | Formal-label/retroactive-promotion inventory and `P -> D -> A` boundary review. Contract and negative modes. | `README.md`; `AGENTS.md`; `scn001_eval/AGENTS.md`; `governance/CONFORMANCE.md`; `governance/sources/`; owner-approval closure below | Source-bound owner review recorded; required CI/protected-check integration remains unverified. | uncovered | claim-blocking; promotion-blocking | Remaining `formal` code/test tokens are the accepted fixture control `named_formal_run` or hostile forbidden-family sentinels, not artifact-status labels; no automated reserved-artifact/authority scan exists yet. |
...
| `ENG-CONF-AUTHORITY-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7B implements immutable qualification plans/results, mechanically derived campaign authorization, outcome-independent attempt and replacement allocation, structural anchor/fresh-start prerequisites, initial invalidity, and owner-reviewed material authority decisions. It deliberately cannot grant execution start before 7C resolves durable namespace and receipt artifacts. | `ADR-004 R3`; `ADR-005 R2`; `ADR-011 R3`; `ADR-012 R3` | Contract, negative, replay, and regression tests over complete slots/comparisons, non-retry, run-count branches, contingency, replacement caps, exact allocation, actor separation, anchor/source/slot binding, and owner-only decisions. | `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/formalAuthority.js`; `scn001_eval/test/formalAuthority.test.js`; Phase 7B record below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | The structural preflight returns `execution_start_authorized: false`; actual authority still requires 7C durable receipt/index resolution, external execution inputs, and independent review of the exact implementation commit. |
| `ENG-CONF-CAPTURE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public inspection passively exposes the bounded chain from proposal/activation through focused and direct correction, delayed-correction activation, later applicability/realization, intervention-conditioned outcome, uncertainty, and explanation support with provenance, typed relations, and sole creating transitions. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Clone-only snapshots; evaluation-private checkpoint reconstruction across production, direct, delayed, later-use, outcome, and explanation closure; replay, malformed-closure, source-binding, relation, cardinality, and participant attacks. Contract, negative, replay-limited modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; bounded SUT closure modules; `scn001_eval/src/productionActiveCheckpoint.js`; `scn001_eval/src/laterOutcomeCheckpoint.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Current capture is passive, run-local, and in-memory through the completed engineering segment; no replay/restore implementation or formal evaluation capture exists. |
...
| `ENG-CONF-EVIDENCE-002` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | not-applicable | Current capture and checkpoints are intentionally in-memory development infrastructure; no durable formal evidence, sealed run/decision/result, campaign index, or authority-namespace index exists. Future trigger: first implementation of any such artifact or operation. | `ADR-002 R2`; `ADR-008 R2`; `ADR-011 R3` | Repository and claim inventory; capture durability and retroactive-promotion review. | Current in-memory capture; `README.md`; `PHASE7_READINESS.md`; accepted ADR-011 snapshot | Not applicable until trigger; then local-recorded plus CI-required controls are mandatory. | N/A | merge-blocking; promotion-blocking; claim-blocking | Applicability must change before durable capture, sealing, indexing, replay, or claim use; all current/historical artifacts remain permanently non-formal. |
...
| `ENG-HEALTH-ABSTRACTION-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Existing bounded SUT/checkpoint abstractions remain unchanged; Phase 7 adds separate evaluation-owned configuration identity and prospective authority modules with no SUT dependency or mutable formal-evidence store. | `ADR-001 R1` | Responsibility review separates canonical envelopes, configuration identity, authority artifacts/decisions, and future durable evidence ownership; dependency gate and hostile suites. | Existing bounded modules; `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/configurationIdentity.js`; `scn001_eval/src/formalAuthority.js`; tests | Implementing-task responsibility review; fresh independent Phase 7A/7B structure review pending. | uncovered | advisory or merge-blocking by condition | New Phase 7 abstractions are locally justified but not independently reviewed; 7C must introduce durable storage without collapsing authority, evidence, and scoring ownership. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Phase 7A configuration identity is committed at `e947dfe`; Phase 7B is a focused prospective-authority increment that creates no durable formal evidence, execution grant, or score. | Rule entry | Focused section diffs; three hostile passes per section; targeted identity/authority suites; full local gates; historical reviews remain scoped to their named ranges. | Phase 7A/7B source/tests and records; prior review closures | Implementing-task red-team and local gates; fresh independent exact-commit reviews pending. | uncovered | advisory or merge-blocking by condition | The implementing task cannot independently attest its Phase 7 commits; protected required-check configuration remains unverified. |
...
| `ENG-HEALTH-FAILURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Malformed or ambiguous state across proposal, both activation chains, focused/direct/later realization, outcome, uncertainty, explanation, provenance, source binding, terminal families, or creators fails closed without staged ingress, partial semantic commit, or repair. | Rule entry | Typed integrity/checkpoint errors; exact-key/reference/envelope/cardinality errors; source, actor, target, timing, scope, creator, result, relation, terminal-family, mismatch, clone, rollback, and replay attacks. | SUT/evaluation source and package tests; `PHASE5_6_PARITY_INVENTORY.md`; repository gates | Default | uncovered | merge-blocking; claim-blocking | Failure transparency and processing rollback cover the completed bounded segment; no operational reporting or durable replay layer exists. |
...
| `ENG-HEALTH-STRUCTURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | SUT closure and checkpoint responsibilities remain unchanged; Phase 7 configuration and authority code is evaluation-owned and cannot mutate SUT state or grant execution without future durable authority resolution. | Rule entry | Static dependency direction; separate modules for identity and authority; generic artifact construction limited to implemented schema families; targeted boundary/authority attacks. | Existing bounded modules; Phase 7A/7B modules and tests; dependency gate | Implementing-task structural review; fresh independent Phase 7 structure review pending. | uncovered | advisory or merge-blocking by condition | New formal-evaluation responsibilities require independent review, and 7C persistence/index integration must preserve this separation. |
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Regressions cover the complete development trajectory, Phase 7A configuration identity, and Phase 7B prospective authority including allocation, qualification, anchoring prerequisites, invalidity/replacement, and material decision attacks. | Rule entry | Existing behavior suites plus Phase 7A/7B contract, negative, property, replay, and regression tests; full aggregate gate. | SUT/evaluation tests; `scn001_eval/test/configurationIdentity.test.js`; `scn001_eval/test/formalAuthority.test.js`; conformance tests; `package.json` | Default | uncovered | merge-blocking; promotion-blocking | Phase 7A/7B mechanisms are locally tested but await fresh independent review; durable evidence and scoring require separate hostile suites. |
