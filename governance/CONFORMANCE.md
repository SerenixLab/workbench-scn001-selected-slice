...
| `ENG-CONF-CONFIG-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7A implements closed V1 behavior/evaluation manifest construction and validation, RFC8785-JCS canonicalization, domain-separated fingerprints, exact typed references/resolution, correction lineage, and fail-closed comparison. The current corrective layer also measures a clean exact Git commit and independently reconstructs both manifests before its conservative three-run authorization constructor can succeed. It creates no effective namespace, stored formal campaign, or execution authority. | `ADR-010 R3`; `ADR-011 R3` | Contract, negative, property, reconstruction, and regression tests over exact schemas, canonicalization, fingerprints, references, recursive source/runtime/policy closure, static imports, environment access, Git provenance, private-layout independence, caller-invented manifest bypass, aliases, and tampering. | `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/configurationIdentity.js`; `scn001_eval/src/configurationMeasurement.js`; `scn001_eval/test/configurationIdentity.test.js`; `scn001_eval/test/configurationMeasurement.test.js`; Phase 7A and corrective records below | Local-recorded tests and full gate; repository CI runs the full gate, but hosted result and protected required-check configuration remain unverified for the corrective head. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Trusted reconstruction is implemented only as a non-public pipeline component; the generic mechanism constructors remain available to synthetic tests, no formal runner enforces the complete chain, no manifest/campaign has been published, and fresh independent exact-commit review remains required before formal reliance. |
...
| `ENG-CONF-SCORE-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7D implements the exact one/three-run branch, closed path/claim obligation matrix, nonnumeric conjunctive aggregation, hard-failure closure, indeterminacy, historical-failure preservation, bounded result/claim ceiling, immutable standing, supersession, and non-circular later-index authority. The corrective execution, qualification, and campaign runners derive actual evidence, comparisons, attempt inventories, and bounded results without caller outcome fields. | `ADR-004 R3`; `ADR-005 R2`; `ADR-009 R4`; `ADR-011 R3`; `ADR-012 R3` | Contract, negative, replay, and adversarial tests cover actual path execution, caller outcome rejection, sealed findings/`NOT_REACHED`, interruption retention, paired qualification/divergence, both complete campaign run-count branches, no voting/weighting, early stopping, independent recomputation, standing forks/terminality, durable supersession chains, and later namespace/receipt authority. | `scn001_eval/src/selectedSliceFormalRunner.js`; `scn001_eval/src/selectedSliceFormalPipeline.js`; `scn001_eval/src/deterministicQualificationRunner.js`; `scn001_eval/src/boundedCampaignRunner.js`; corresponding tests; `scn001_eval/src/boundedScoring.js`; Phase 7D and corrective records below | Local-recorded targeted/full gates and repository CI; protected required-check configuration and fresh exact-commit independent review remain pending. | uncovered | merge-blocking; promotion-blocking; claim-blocking | The internal implementation pipeline closes through a bounded result, but every current execution is synthetic and no real project-owner standing, protected integration, independent exact-commit review, or formal claim exists. |
...
`PHASE7_IMPLEMENTATION_COMPLETE_EXTERNAL_CLOSURE_PENDING`. All objects remain
synthetic and non-promotable. Fresh exact-commit independent review, protected
required-check verification/integration, real project-owner key/effective
namespace designation and external events, and the human standing decision are
still required before formal reliance. The four Phase 7 controls remain
`applicable / uncovered` until those external closures are recorded.
