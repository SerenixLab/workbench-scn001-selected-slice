...
| `ENG-CONF-AUTHORITY-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | not-applicable | No qualification plan/result, campaign authorization, external anchor/fresh-start chain, formal attempt allocation, or post-observation authority decision exists. Future trigger: first implementation of any such artifact or operation. | `ADR-004 R3`; `ADR-005 R2`; `ADR-011 R3`; `ADR-012 R3` | Repository and claim inventory; accepted-source prospectivity review. | `README.md`; `PHASE7_READINESS.md`; accepted source snapshots | Not applicable until trigger; then local-recorded plus CI-required and qualifying review are mandatory. | N/A | merge-blocking; promotion-blocking; claim-blocking | Applicability must change before the first prospective authority artifact or operation; no current development artifact can satisfy the trigger retroactively. |
...
| `ENG-HEALTH-ABSTRACTION-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Private retained-state, proposal, activation, focused, direct, delayed, later-use, later-outcome, evaluation-checkpoint, and source-binding modules are non-throwaway bounded internal abstractions. | `ADR-001 R1` | Each module owns one named closure or passive checkpoint; `RunState` alone owns mutable state and orchestration; evaluation checkpoints own no behavior. Complete responsibility and parity review covered the corrected Phase 5–6 implementation and fixture-initialization hardening. | Bounded SUT/evaluation modules; package tests; `PHASE5_6_PARITY_INVENTORY.md`; independent closures for `4376bcc..6e57840` and `6e57840..6d8b41d` below | Qualifying independent implementation review recorded below. | review-only | advisory or merge-blocking by condition | Current bounded abstractions are justified and reviewed; future generalization or a new ownership boundary requires fresh justification. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Phase 7A is a focused evaluation-owned configuration-identity increment over approved implementation baseline `e6e1dc1`; it does not alter SUT behavior, authorize a campaign, capture formal evidence, or aggregate a result. | Rule entry | Focused diff; three hostile implementation passes; targeted configuration tests; full local gate; historical reviews remain scoped to their named ranges. | Phase 7A source/tests; Phase 7A record below; prior review closures | Implementing-task red-team and local gates; fresh independent exact-commit review pending. | uncovered | advisory or merge-blocking by condition | The implementing task cannot independently attest its own Phase 7A commit; protected required-check configuration remains unverified. |
...
| `ENG-HEALTH-STRUCTURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | SUT closure modules keep focused, direct, delayed, later-use, and later-outcome responsibilities below `RunState`, while evaluation checkpoint modules remain separate from transport provenance and simulator realization. | Rule entry | Dependency direction remains evaluation-to-public-SUT only; `RunState` alone owns mutable state, order, commits, lifecycle, and orchestration; checkpoint modules are passive and own no behavior. | Bounded SUT/evaluation modules; `scn001_sut_core/src/runState.js`; `scn001_eval/src/harness.js`; package tests; independent closures below | Qualifying independent structure review recorded below. | review-only | advisory or merge-blocking by condition | Current responsibilities are independently reviewed; future generalization, persistence, or a second mutable owner requires fresh structure review. |
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Regressions cover the full development public-boundary trajectory and Phase 7A configuration identity, including canonicalization, closed schema/reference vocabulary, tampering, alias, unknown-input, lineage, applicability, and material-change attacks. | Rule entry | Existing positive/hostile behavior suites plus Phase 7A contract, negative, property, and regression tests; full aggregate gate. | SUT/evaluation tests; `scn001_eval/test/configurationIdentity.test.js`; conformance tests; `package.json`; review records | Default | uncovered | merge-blocking; promotion-blocking | Phase 7A mechanisms are locally tested but await fresh independent review; protected required-check configuration remains unverified and later authority/evidence/scoring sections require separate regressions. |
