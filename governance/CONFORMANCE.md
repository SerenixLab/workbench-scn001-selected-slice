...
| `ENG-CONF-RUN-001` | `R3` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public run lifecycle, closure-owned source, candidate, proposal, activation-assessment, and active-trial identities, harness-owned per-run fixture mappings, and cross-run tests exist. | `ADR-003 R2`; `ADR-004 R3`; `ADR-008 R2` | Closure-owned run map; independent-run identity isolation; retained activation and active-trial refs become unresolvable after termination. Negative and replay-limited modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_sut_core/test/run-state.test.js`; `scn001_eval/src/harness.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | Active-trial isolation is covered for the bounded production activation seam; outcome state and later behavior remain absent. |
...
| `ENG-CONF-STATE-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Proposal realization, binding, activation assessment, and active-trial creation consume revalidated exact participant closures with contemporaneous typed relations and order. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Transition/ref/order/role assertions; retained-input immutability attacks; missing/duplicate/competing basis attacks; exact replay and active-trial closure. Contract and negative modes. | `scn001_sut_core/src/runState.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | The bounded production activation mutation is covered; later-use applicability, behavior, outcome, and later lifecycle mutations remain absent. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The current corrective increment closes four activation and active-trial integrity defects without adding later behavior. | Rule entry | Focused-diff inspection against starting `HEAD` `4ac60ee`; one primary purpose: exact activation participant, provenance, conflict, and active-trial closure. | Current working-tree diff; package tests; corrective record below | The independent review of `4ac60ee` is blocking/failed; fresh independent ChatGPT review of the corrected pushed diff is pending. | uncovered | advisory or merge-blocking by condition | Prior passing reviews remain historical and are not reused; the corrected diff requires fresh change-specific review. |
