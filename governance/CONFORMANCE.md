...
| `ENG-CONF-CONFIG-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Phase 7A implements closed V1 behavior/evaluation manifest construction and validation, RFC8785-JCS canonicalization, domain-separated fingerprints, exact typed references/resolution, correction lineage, and fail-closed comparison. It creates no stored campaign artifact or formal authority. | `ADR-010 R3`; `ADR-011 R3` | Contract, negative, property, and regression tests over exact schemas, canonicalization, fingerprints, references, source/runtime/policy closure, fixture path/claim closure, provenance, aliases, and tampering. | `scn001_eval/src/formalArtifactIdentity.js`; `scn001_eval/src/configurationIdentity.js`; `scn001_eval/test/configurationIdentity.test.js`; Phase 7A record below | Local-recorded tests and full gate; repository CI runs the full gate, but protected required-check configuration remains unverified. | uncovered | merge-blocking; promotion-blocking; claim-blocking | Builders validate caller-supplied closed identity assertions but do not independently inventory repository bytes; no manifest has been published or authorized, and fresh independent review of the exact Phase 7A commit remains required before formal reliance. |
