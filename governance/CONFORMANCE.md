...
| `ENG-CONF-DEP-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Proposal, binding, activation, and active-trial ancestry require exact contemporaneous typed transition, basis, ingestion, control-provenance, and relation evidence. | `ADR-007 R3`; `ADR-008 R2` | Exact candidate ancestry; complete two-interaction activation-control inventory; role-qualified redelivery; exact first/current ingestion participants and basis; exact creating-trigger transition/interaction; causal-order and active-trial ancestry attacks. Contract, negative, and replay modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/test/run-state.test.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | The bounded production-activation lineage is covered; generic future-family lineage and lifecycle attacks remain incomplete. |
...
| `ENG-CONF-INSPECT-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Inspection passively exposes proposal/realization/binding closure plus the exact two-interaction activation-control basis, role-qualified relations, assessment, and active trial. | `ADR-006 R2`; `ADR-008 R2` | Clone-only inspection; emission invariance; complete malformed-control/provenance/trigger closure attacks never repair state. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/test/run-state.test.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | Bounded activation inspection is covered; outcome and later lifecycle history remain absent. |
...
| `ENG-CONF-PAYLOAD-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Boundary and retained-state validation complete before outward emission, simulator invocation, projection, activation assessment, or active-trial mutation. | `ADR-005 R2`; `ADR-006 R2`; `ADR-008 R2` | Pre-emission proposal closure; closed projection; atomic ingress; unknown-control and exact-trigger no-mutation snapshots; malformed state fails before behavior or activation mutation. Negative and contract modes. | `scn001_sut_core/src/inputValidation.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | No deserializer or telemetry path exists; future rejection paths need equivalent atomicity checks. |
...
| `ENG-CONF-REF-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Proposal ancestry is the sole activation-candidate authority; control conflicts/redelivery use exact fact identities; active-trial identity equals its assessment and exact trigger interaction. | `ADR-005 R2`; `ADR-007 R3`; `ADR-008 R2` | UUID-backed opaque refs; distinct-equal control, equal-scope candidate, fake-transition, interaction-retargeting, consistently rewritten ancestry, and cross-run attacks. | `scn001_sut_core/src/runState.js`; `scn001_eval/src/simulatorProjection.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Candidate, fact, assessment, trial, interaction, and realization identities are exact and run-scoped; later-use references remain future work. |
| `ENG-CONF-ROLE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Evaluation adapters preserve declared roles, and activation replay requires exact semantic-source actors plus separate candidate/binding logical roles for every control identity. | `ADR-005 R2`; `ADR-006 R2` | Full fixture/simulator source validation; wrong-actor attacks; region-qualified control relations; dual-role shared consequence/reversibility relation-removal attacks. | `scn001_eval/src/fixtureProjection.js`; `scn001_eval/src/simulatorProjection.js`; `scn001_sut_core/src/runState.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Current activation roles, origins, actors, and identities are covered; future roles require equivalent attacks. |
...
| `ENG-CONF-STATE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Selection, binding, activation, and output/trial availability resolve exact retained identities without ranking, arbitration, first/latest control selection, or loose relation predicates. | `ADR-004 R3`; `ADR-006 R2`; `ADR-007 R3` | Exact proposal/selection/realization resolution plus complete co-applicable control inventory, identity-based conflict/redelivery, material-context basis, and malformed closure attacks. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/test/run-state.test.js` | Default | uncovered | merge-blocking; claim-blocking | Bounded activation resolution is narrow and exact; later-use applicability and other state families remain unimplemented. |
| `ENG-CONF-STATE-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Activation and active-trial creation consume one exact binding/proposal/candidate/interaction chain, the complete co-applicable control basis, and exact creating triggers with complete retained-fact provenance and causal order. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Exact transition/ref/order/role assertions; unknown/conflicting/distinct-equal control attacks; dual-role redelivery; material-context basis; fake-trigger, equal-shape substitution, interaction retargeting, semantic actor, ingestion, replay, and ancestry attacks. Contract and negative modes. | `scn001_sut_core/src/runState.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | The bounded production activation mutation is covered; later-use applicability, behavior, outcome, and later lifecycle mutations remain absent. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The current corrective increment closes only the reviewed activation-control inventory, exact-basis, and pre-mutation trigger defects plus the adjacent material-context basis omission. | Rule entry | Focused-diff inspection against starting `HEAD` `0106c17e5400c0261fcc03baa0438bad44131d07`; two-interaction control closure, exact role-qualified provenance, and creating-transition/interaction trigger validation. | Current working-tree diff; package tests; corrective record below | Independent review of `0106c17e5400c0261fcc03baa0438bad44131d07` is blocking/failed; fresh independent ChatGPT review of the corrected pushed diff is pending. | uncovered | advisory or merge-blocking by condition | Prior passing and failed reviews remain historical; the corrected diff requires fresh change-specific review. |
...
| `ENG-HEALTH-FAILURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Malformed proposal, realization, full simulator record, activation control, provenance, trigger, reference, and multiplicity states fail closed rather than suppressing evidence, mutating trial state, or satisfying replay. | Rule entry | Typed SUT integrity errors; exact-key/type/reference errors; unknown-control, fake-trigger, wrong-interaction, override/cardinality, and negative replay attacks. | SUT/evaluation source and package tests; repository gates | Default | uncovered | merge-blocking; claim-blocking | Failure transparency is covered for the bounded seam; no operational reporting layer exists. |
...
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Regressions cover complete two-interaction controls, exact-identity conflicts/redelivery, basis/provenance removal, fake triggers, wrong interactions, substitution, ingestion, replay, and run isolation. | Rule entry | Positive formal-harness plus negative unknown, missing, unsupported, conflicting, distinct-equal, redelivery, material-context, exact-interaction, no-mutation, causal-order, actor-authority, ingestion-basis, passivity, replay, and non-scope modes. | SUT/evaluation tests; conformance tests; `package.json` | Default | uncovered | merge-blocking; promotion-blocking | The bounded activation closure is covered; later use, behavior, outcome, and formal evaluation remain future-triggered. |
...
not self-attest a passing review. Remaining risks include that pending review,
manual test-validity review, unverified protected required-check configuration,
and the intentionally absent later semantic families. No focused-drill delivery
or behavior, later-use applicability, direct correction, delayed-correction
state, outcome, explanation, formal evaluation, scoring, completion, broader
`SCN-001` acceptance, or production-readiness artifact is added.
