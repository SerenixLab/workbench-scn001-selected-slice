...
| `ENG-CONF-DEP-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Current SUT transitions create source, basis, and support relations consumed by inspection snapshots; exact candidate support and no-comparison dispositions preserve contemporaneous typed basis relations. | `ADR-007 R3`; `ADR-008 R2` | Contemporaneous relation creation, once-per-interaction canonical fact participation, exact complete comparison-basis verification, candidate affordance/comparison lineage, exact no-comparison observation/affordance basis, and passive inspection tests. Contract and negative-limited modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/test/run-state.test.js` | Default | uncovered | merge-blocking; claim-blocking | Same-batch multiplicity, exact-basis subset, shared-observation, ambiguity, replay, and contemporaneous candidate/disposition-lineage attacks are covered; generic missing-lineage, backdated-reporting, post-hoc-basis, and later-snapshot attacks remain incomplete. |
...
| `ENG-CONF-REF-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Run, source-fact, actor, state, transition, interaction, and relation references cross public and inspection surfaces. | `ADR-005 R2`; `ADR-007 R3`; `ADR-008 R2` | UUID-backed opaque source/state references; evaluation-owned per-run fixture mapping; separate SUT source-to-input binding; identity-based first-occurrence semantic normalization; literal evaluation-ID, collision, equal-payload-distinction, and cross-run negative tests. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/inputValidation.js`; `scn001_sut_core/test/run-state.test.js`; `scn001_eval/src/fixtureProjection.js`; `scn001_eval/src/harness.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | Source identity and SUT state identity are separate and purpose-free; positional acceptance correlation is distinct from semantic evidence multiplicity; future accepted later-use state references still require change-specific semantic-leakage review. |
...
| `ENG-CONF-STATE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Current processing resolves unique canonical run-local source facts, exact dimension comparisons, and exact no-comparison observation bases through bounded identity access and narrow typed local relations. | `ADR-004 R3`; `ADR-006 R2`; `ADR-007 R3` | Run-local source-to-input map; first-occurrence semantic normalization; incoming role-qualified basis-relation intersection; complete exact basis equality; exact no-comparison observation-set matching; zero-match withholding; multiple-match integrity failure; no retrieval/ranking API. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/test/run-state.test.js` | Default | uncovered | merge-blocking; claim-blocking | Current comparison and no-comparison basis resolution is selected-slice-specific and exact; later-use applicability and other state families remain unimplemented and require their own bounded access review. |
...
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Source identity, canonical retention, attribution, temporal, comparison, candidate, no-comparison disposition, boundary, and failure behavior changed with fail-without-it attacks. | Rule entry | Positive, negative, contract, regression, replay, ambiguity, atomicity, same-batch multiplicity, equal-payload identity, exact incomplete-basis, contemporaneity, interleaving, and run-isolation tests cover the stabilization and corrective categories. | `scn001_sut_core/test/run-state.test.js`; `scn001_eval/test/harness.test.js`; `scn001_sut_core/test/public-boundary.test.js`; `tests/conformance/`; `package.json` | Default | uncovered | merge-blocking; promotion-blocking | Current tests cover the implemented stabilization and the two reviewed blockers; future behavior changes still require change-specific fail-without-it evidence. |
| `ENG-HEALTH-TEST-002` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Tests are used throughout this ledger as implementation and conformance evidence. | Rule entry | Public-boundary attacks exercise duplicate delivery, processing, aggregation, replay, changed identity basis, and inspection interleaving; a direct `RunState` attack reaches the complete-group/zero-comparison branch that normal processing immediately satisfies; static contract tests remain separate. | `scn001_sut_core/test/`; `scn001_eval/test/`; `tests/conformance/` | Default | uncovered | merge-blocking; claim-blocking | Test-validity review is not independently recorded; the direct zero-comparison branch attack is implementation-level evidence, and no test is behavioral-compatibility evidence. |
