...
| `ENG-CONF-CAPTURE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public inspection passively exposes proposal/activation closure plus focused-drill instruction, disposition, realization, outcome, provenance, typed relations, and sole creating transitions. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Clone-only snapshots; evaluation-private independent production-active reconstruction; replay, malformed-closure attacks, and outcome participant inspection. Contract, negative, replay-limited modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/focusedDrill.js`; `scn001_sut_core/test/run-state.test.js`; `scn001_eval/src/productionActiveCheckpoint.js`; `scn001_eval/src/harness.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | Current in-memory focused-drill capture is passive for the bounded family; no replay/restore implementation or formal evaluation capture exists. |
...
| `ENG-CONF-HARNESS-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | The formal one-argument harness transports one exact SUT-selected proposal or focused-drill request and gates D-001/D-002 and D-003/D-004 through passive exact checkpoints. | `ADR-008 R2` | Evaluation-private CP-PROD-ACTIVE reconstruction independently derives all nine results, reasons, overall status, control inventories, material refs, participant roles, and complete ancestry before ingress; CP-DRILL-REALIZATION-MATCH remains exact. Contract and negative modes. | `scn001_eval/src/productionActiveCheckpoint.js`; `scn001_eval/src/harness.js`; `scn001_eval/src/simulator.js`; `scn001_eval/src/simulatorProjection.js`; `scn001_eval/index.js`; `scn001_eval/test/harness.test.js` | Default | uncovered | merge-blocking; claim-blocking | Non-arbitration and governed transport are covered through the focused-drill seam; direct correction and later simulator families require separate contracts. |
...
| `ENG-CONF-INSPECT-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Inspection passively exposes proposal/activation and focused-drill instruction/disposition/realization/outcome closure with exact typed relations and creators. | `ADR-006 R2`; `ADR-008 R2` | Clone-only inspection; independent activation reconstruction from snapshots; emission invariance; exact claimant counts; malformed checkpoint/focused participant attacks never repair state or reach fixture ingress. Contract and negative modes. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/focusedDrill.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_eval/src/productionActiveCheckpoint.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Bounded focused-drill inspection is covered; direct-correction and later lifecycle history remain absent. |
...
| `ENG-CONF-PAYLOAD-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Boundary and retained-state validation complete before fixture ingress, outward emission, simulator invocation, projection, activation assessment, or focused mutation. | `ADR-005 R2`; `ADR-006 R2`; `ADR-008 R2` | Independent pre-ingress checkpoint reconstruction; pre-emission full active-foundation closure; atomic ingress; exact ingestion and competing-creator no-mutation snapshots. Negative and contract modes. | `scn001_sut_core/src/inputValidation.js`; `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; private closure modules; `scn001_eval/src/productionActiveCheckpoint.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | No deserializer or telemetry path exists; future rejection paths need equivalent atomicity checks. |
...
| `ENG-CONF-REF-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Proposal ancestry is the sole activation-candidate authority; controls use exact fact identities; assessment/trial creator pointers and focused ingestion evidence must agree with exactly one retained transition claimant. | `ADR-005 R2`; `ADR-007 R3`; `ADR-008 R2` | UUID-backed opaque refs; run-owned original interaction/ingestion binding; distinct-equal control, fake/clone transition, creator-pointer disagreement, interaction retargeting, rewritten ancestry, and cross-run attacks. | `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_sut_core/src/proposalClosure.js`; `scn001_sut_core/src/focusedDrill.js`; `scn001_eval/src/simulatorProjection.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Candidate, fact, assessment, trial, transition, interaction, and realization identities are exact and run-scoped; later-use references remain future work. |
...
| `ENG-CONF-RUN-001` | `R3` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Public run lifecycle keeps source, canonical first-ingestion, activation, focused-drill instruction/disposition/realization, and short-term outcome identities isolated across independent runs. | `ADR-003 R2`; `ADR-004 R3`; `ADR-008 R2` | Run-owned source bindings anchor original ingestion identity; independent runs create distinct focused refs; foreign refs are absent; exact replay reuses same-run semantic closure. Negative and replay modes. | `scn001_sut_core/src/publicBoundary.js`; `scn001_sut_core/src/runState.js`; `scn001_sut_core/src/retainedStateClosure.js`; `scn001_eval/src/harness.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Focused-drill isolation is explicit; direct correction and later-use state remain absent. |
...
| `ENG-HEALTH-ABSTRACTION-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Private retained-state, proposal, production-activation, focused-drill, and evaluation production-active checkpoint modules are non-throwaway bounded internal abstractions. | `ADR-001 R1` | `focusedDrill.js` remains read-only derivation/validation with mutation in `RunState`; `productionActiveCheckpoint.js` is a passive evaluation-private reconstruction helper, not a generic graph or policy framework. | `scn001_sut_core/src/focusedDrill.js`; existing private SUT modules; `scn001_sut_core/src/runState.js`; `scn001_eval/src/productionActiveCheckpoint.js`; package tests; corrective addendum below | Fresh independent abstraction review of the corrected commit is pending; historical reviews remain preserved below. | uncovered | advisory or merge-blocking by condition | Both bounded modules have implementing-task review only for this correction; fresh qualifying independent review is required. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The current change has one primary purpose: correct exact checkpoint and retained-lineage gaps in the accepted bounded focused-drill trajectory. | Rule entry | Focused diff against corrective baseline `d54a8fb7944b92883a0d934ffe0f01b2634a21fa`; thirty requested attack classes plus adjacent first-ingestion rewrite attack; full local gates; hostile implementing-task review. | Current corrective commit/diff; SUT/evaluation implementation and tests; failed baseline review and corrective addendum below | Independent review of `d54a8fb7944b92883a0d934ffe0f01b2634a21fa` failed; fresh independent review of the corrected pushed commit is pending. | uncovered | advisory or merge-blocking by condition | The implementing task cannot independently close its own change-review condition. |
...
| `ENG-HEALTH-FAILURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Malformed proposal, activation, focused-drill foundation, realization, outcome, provenance, target, and competing creator states fail closed without fixture ingress or semantic repair. | Rule entry | Typed integrity/checkpoint errors; exact-key/reference errors; wrong actor/target, recomputation mismatch, retarget, clone, missing/duplicate basis, pre-mutation, and replay attacks. | SUT/evaluation source and package tests; repository gates | Default | uncovered | merge-blocking; claim-blocking | Failure transparency is covered for the bounded focused-drill seam; no operational reporting layer exists. |
...
| `ENG-HEALTH-STRUCTURE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The SUT keeps bounded closure modules under `RunState`, while evaluation keeps independent production-active checkpoint reconstruction beside the harness. | Rule entry | `focusedDrill.js` validates focused participant structures through a read-only `RunState` authority callback; `productionActiveCheckpoint.js` owns only passive CP-PROD-ACTIVE reconstruction; mutation, allocation, commit, lifecycle, public API, and later families remain outside both helpers. | `scn001_sut_core/src/focusedDrill.js`; `scn001_sut_core/src/runState.js`; `scn001_eval/src/productionActiveCheckpoint.js`; package tests; corrective addendum below | Fresh independent structure review of the corrected commit is pending; prior reviews remain historical. | uncovered | advisory or merge-blocking by condition | The corrected module boundaries have only implementing-task review and require fresh independent review. |
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Regressions cover both focused checkpoints, independent nine-check reconstruction, complete active-trial foundations, exact ingestion evidence, five-object separation, mismatch, no-mutation, replay, run isolation, and denied later families. | Rule entry | Positive formal-harness trajectory plus the thirty requested checkpoint/foundation/realization/outcome attacks and adjacent coordinated first-ingestion rewrite attack. | SUT/evaluation tests; conformance tests; `package.json` | Default | uncovered | merge-blocking; promotion-blocking | The bounded focused-drill family is covered; direct correction and every later family remain future-triggered. |
| `ENG-HEALTH-TEST-002` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The canonical focused-drill trajectory uses the formal harness and public boundary; direct `RunState` use is limited to malformed retained-state, foundation, ingestion, replay, isolation, and unreachable pre-mutation attacks. | Rule entry | End-to-end checkpoint delivery and simulator routing; pre-ingress counters for inspection corruption; raw fixtures contain no SUT conclusions; direct tests attack exact internal closure without becoming behavior evidence. | `scn001_sut_core/test/`; `scn001_eval/test/`; `tests/conformance/` | Default | uncovered | merge-blocking; claim-blocking | Fresh independent test-validity review is pending; tests remain engineering/conformance evidence only. |
