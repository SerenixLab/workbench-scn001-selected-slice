...
| `ENG-CONF-REF-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Proposal ancestry is the sole activation-candidate authority, and active-trial candidate identity exactly equals its assessment candidate. | `ADR-005 R2`; `ADR-007 R3`; `ADR-008 R2` | UUID-backed opaque refs; equal-scope/equal-payload assessment substitution; consistently rewritten trial transition/ancestry; interaction-retargeting and cross-run attacks. | `scn001_sut_core/src/runState.js`; `scn001_eval/src/simulatorProjection.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Candidate, assessment, trial, interaction, and realization identities are exact and run-scoped; later-use references remain future work. |
| `ENG-CONF-ROLE-001` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Evaluation adapters preserve declared roles, and activation replay requires each relevant fact's exact role-specific semantic-source actor. | `ADR-005 R2`; `ADR-006 R2` | Full fixture/simulator role/source validation; wrong-actor control, chronology, context, observation, response, and realization attacks, including internally consistent source closure. | `scn001_eval/src/fixtureProjection.js`; `scn001_eval/src/simulatorProjection.js`; `scn001_sut_core/src/runState.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | Current activation roles, origins, actors, and identities are covered; future roles require equivalent attacks. |
...
| `ENG-CONF-STATE-002` | `R2` | `profiles/SCN001_SELECTED_SLICE.md` | applicable | Activation and active-trial creation consume one exact binding/proposal/candidate/interaction chain with complete retained-fact ingestion provenance and causal order. | `ADR-006 R2`; `ADR-007 R3`; `ADR-008 R2` | Exact transition/ref/order/role assertions; equal-shape substitution, interaction retargeting, semantic actor, first-ingestion, current-ingestion, replay, and ancestry attacks. Contract and negative modes. | `scn001_sut_core/src/runState.js`; package tests | Default | uncovered | merge-blocking; claim-blocking | The bounded production activation mutation is covered; later-use applicability, behavior, outcome, and later lifecycle mutations remain absent. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The current corrective increment closes only the three remaining activation identity, interaction/provenance, and first-ingestion defects. | Rule entry | Focused-diff inspection against starting `HEAD` `c90d279`; exact assessment/trial candidate identity, binding interaction identity, semantic actors, and ingestion closure. | Current working-tree diff; package tests; corrective record below | Independent review of `c90d279` is blocking/failed; fresh independent ChatGPT review of the corrected pushed diff is pending. | uncovered | advisory or merge-blocking by condition | Prior passing and failed reviews remain historical; the corrected diff requires fresh change-specific review. |
...
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Regressions cover all requested assessment/trial substitution, interaction retargeting, semantic-actor, first-ingestion, replay, and run-isolation attacks. | Rule entry | Positive formal-harness plus negative equal-shape identity, exact-interaction, causal-order, actor-authority, ingestion-basis, passivity, replay, and non-scope modes. | SUT/evaluation tests; conformance tests; `package.json` | Default | uncovered | merge-blocking; promotion-blocking | The bounded activation closure is covered; later use, behavior, outcome, and formal evaluation remain future-triggered. |
