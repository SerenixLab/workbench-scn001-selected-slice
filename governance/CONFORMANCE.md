...
| `ENG-CONF-CONFIG-001` | `R1` | `profiles/SCN001_SELECTED_SLICE.md` | not-applicable | No V1 behavior/evaluation manifest, canonical fingerprint implementation, formal configuration comparison, correction, supersession, or exact formal reference exists. Future trigger: first implementation of any such artifact or operation. | `ADR-010 R3`; `ADR-011 R3` | Repository and claim inventory; accepted manifest/reference vocabulary review. | `README.md`; `PHASE7_READINESS.md`; accepted ADR-010/011 snapshots | Not applicable until trigger; then local-recorded plus CI-required controls are mandatory. | N/A | merge-blocking; promotion-blocking; claim-blocking | Applicability must change before the first manifest, canonicalization, comparison, or exact-reference implementation; current commits and package versions are not manifest identity. |
...
| `ENG-HEALTH-CHANGE-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | The completed implementation scope is the bounded engineering chain through `DP-OUTCOME-UPDATE` and `DP-EXPLAIN`, including the reviewed Phase 5–6 corrective and fixture-initialization hardening ranges. The ledger correction and this approval closure change governance records only. | Rule entry | Historical implementation diffs and independent closures; focused ledger diff against base `ac7fbb7`; qualifying independent review of exact correction commit `2385020`; source-bound owner approval; full local gate. | Current implementation/tests; review closures below; `governance/CONFORMANCE.md`; `PHASE7_READINESS.md` | Qualifying implementation and ledger-correction reviews recorded below. | review-only | advisory or merge-blocking by condition | Implementation scope and the focused ledger correction are independently reviewed; future governed changes require fresh review as applicable, and protected required-check configuration remains unverified. |
...
| `ENG-HEALTH-TEST-001` | `R2` | `ENGINEERING_STANDARD.md` | applicable | Regressions cover the full development public-boundary trajectory through outcome and explanation, plus fixture-initialization completeness, exact source/creator/envelope/cardinality closure, rollback, no-mutation, replay, run isolation, and claim-boundary denials. | Rule entry | Positive development public-boundary trajectory plus missing, duplicate, substituted, competing, wrong actor/target/scope/timing, malformed relation/transition/order, mismatch, creator, terminal-family, causal-overclaim, rollback, replay, and independent-run attacks. | SUT/evaluation tests; conformance tests; `package.json`; review closures below | Default | uncovered | merge-blocking; promotion-blocking | The completed bounded behavior is tested and independently reviewed; protected required-check configuration remains unverified and future behavior requires new regressions. |
