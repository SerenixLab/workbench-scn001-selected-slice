...
The implemented chain now passes `CP-PROD-ACTIVE`, the bounded `DP-FOCUSED-DRILL` trajectory through `CP-DRILL-REALIZATION-MATCH`, the bounded `DP-DIRECT-CORRECTION` trajectory through `CP-DIRECT-CORRECTION-REALIZED`, and `DP-LATER-USE` plus `DP-REALIZATION` through `CP-REALIZATION-FIDELITY`. The current user correction changes behavior immediately in the exact current spontaneous session by selecting turn-completion correction timing; it does not request immediate linguistic correction. The prior focused-drill immediate-correction instruction remains valid historical evidence for its different exact scope. Neither instruction is global or cross-surface.
...
At D10, the SUT rechecks that same active trial against exact current context, same-run state reference, and user-governed/consequence controls. The canonical spontaneous-production branch creates a separate applicable assessment and bounded disposition, then records an exact matched simulator realization without changing the active trial or establishing a global policy. `CF-DRILL-OPT-IN` instead records explicit `not_applicable` for the focused drill with immediate-correction opt-in; it creates no delayed disposition and does not fabricate narrowing, supersession, or retirement. The workbench does not implement the later outcome, explanation, formal evaluation, or scoring families.
...
The boundary spine and selected semantic chain are implemented through exact `CP-REALIZATION-FIDELITY`. The next bounded semantic frontier is the later outcome and canonical-intervention path after `DP-REALIZATION`.
...
3. preserve the delayed-correction candidate as immutable non-active state and preserve its separate activation assessment, active trial, later-use assessment, disposition, and realization;
4. admit `L-003` through `L-005` only after exact `CP-REALIZATION-FIDELITY`, without treating the outcome as causal proof.

Later outcome, explanation, formal-evaluation, and scoring families remain unimplemented. No current artifact establishes behavioral compatibility, formal evidence, scoring, milestone acceptance, broader `SCN-001` acceptance, or production readiness.
