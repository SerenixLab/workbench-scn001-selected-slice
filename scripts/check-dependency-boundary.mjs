...
export function assertEvaluationImportsPublicSutOnly(file, contents, { forbidDynamicLoaders = false } = {}) {
  if (forbidDynamicLoaders && /\bimport\s*\(|\b(?:require|createRequire)\s*\(/.test(contents)) {
    throw new Error(
      `ENG-CONF-IMPORT-002: ${relative(repositoryRoot, file)} uses a forbidden dynamic module loader.`
    );
  }
...
export async function runDependencyBoundaryCheck() {
  const [sutFiles, evaluationSourceFiles, evaluationTestFiles, sutPackage, evaluationPackage] = await Promise.all([
    collectJavaScriptFiles(sutRoot),
    collectJavaScriptFiles(resolve(evaluationRoot, "src")),
    collectJavaScriptFiles(resolve(evaluationRoot, "test")),
    readPackage(sutRoot),
  ...
  for (const file of evaluationSourceFiles) {
    assertEvaluationImportsPublicSutOnly(file, await readFile(file, "utf8"), { forbidDynamicLoaders: true });
  }

  for (const file of evaluationTestFiles) {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  await runDependencyBoundaryCheck();
}
